...
// ---------------------------------------------------------------------------
// global fetch — required by @apollo/client HttpLink (avoids Invariant
// Violation "fetch is not found globally" when client.ts is imported)
// ---------------------------------------------------------------------------
...
jest.mock('next/link', () => {
    ...
    const MockLink = ({ children, href, ...rest }) => {
        return React.createElement('a', { href: typeof href === 'object' ? href.pathname : href, ...rest }, children);
    };
