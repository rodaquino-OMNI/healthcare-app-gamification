...
/**
 * Hook for fetching activity-type health metrics for the current user.
 * Wraps getHealthMetrics with types=['activity'] and supports an optional
 * date range (defaults to the last 30 days).
 *
 * @param dateRange - Optional start/end date range for filtering metrics
 * @returns Object containing activity data, loading state, error, and refetch
 */
export const useActivity = (dateRange?: DateRange): UseActivityReturn => {
    ...
    const fetchData = useCallback(() => {
        ...
        getHealthMetrics(userId, ['activity'], range.start, range.end)
            .then((result) => {
                if (!cancelled) {
                    setData(result);
                }
