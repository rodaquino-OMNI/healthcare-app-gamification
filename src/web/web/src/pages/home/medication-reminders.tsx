  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <a href="/health/medications" style={styles.viewAll}>Ver Todos</a>
      </div>
          ...
          {reminders.map((med) => (
            <div key={med.id} style={styles.card}>
              ...
              <div style={styles.actions}>
                <button style={styles.takeBtn} onClick={() => handleTake(med.id)}>Tomar</button>
                <button style={styles.skipBtn} onClick={() => handleSkip(med.id)}>Pular</button>
              </div>
            </div>
          ))}
        </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  viewAll: { fontSize: typography.fontSize['text-sm'], fontWeight: typography.fontWeight.semiBold, color: colors.brand.primary, textDecoration: 'none' },
  list: { display: 'flex', flexDirection: 'column', gap: spacing.sm },
  card: { display: 'flex', alignItems: 'center', backgroundColor: colors.neutral.white, borderRadius: borderRadius.md, padding: spacing.md, borderLeft: `4px solid ${colors.journeys.health.primary}`, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' },
  pillIcon: { fontSize: typography.fontSize['heading-xl'], marginRight: spacing.md },
  info: { flex: 1 },
  drugName: { fontSize: typography.fontSize['text-md'], fontWeight: typography.fontWeight.bold, color: colors.neutral.gray900, margin: 0, fontFamily: typography.fontFamily.body },
  dosage: { fontSize: typography.fontSize['text-sm'], color: colors.neutral.gray600, margin: `${spacing['4xs']} 0`, fontFamily: typography.fontFamily.body },
  time: { fontSize: typography.fontSize['text-sm'], fontWeight: typography.fontWeight.semiBold, color: colors.journeys.health.primary, margin: 0, fontFamily: typography.fontFamily.body },
  actions: { display: 'flex', flexDirection: 'column', gap: spacing['3xs'], alignItems: 'center' },
  takeBtn: { backgroundColor: colors.journeys.health.primary, color: colors.neutral.white, border: 'none', borderRadius: borderRadius.sm, padding: `${spacing.xs} ${spacing.md}`, fontSize: typography.fontSize['text-xs'], fontWeight: typography.fontWeight.bold, cursor: 'pointer', fontFamily: typography.fontFamily.body },
  skipBtn: { backgroundColor: 'transparent', color: colors.neutral.gray500, border: 'none', fontSize: typography.fontSize['text-xs'], cursor: 'pointer', fontFamily: typography.fontFamily.body },
