...
const MOCK_MESSAGES: AsyncMessage[] = [
  { id: 'a1', sender: 'patient', text: 'Hi Dr. Santos, I wanted to let you know the headaches have improved since starting the medication.', time: '9:15 AM', date: 'Feb 20', read: true },
  { id: 'a2', sender: 'doctor', text: 'That is great news! Are you experiencing any side effects from the ibuprofen or cyclobenzaprine?', time: '11:30 AM', date: 'Feb 20', read: true },
  { id: 'a3', sender: 'patient', text: 'Some mild drowsiness from the cyclobenzaprine, but nothing too bad. Should I continue at the same dose?', time: '2:45 PM', date: 'Feb 20', read: true },
  { id: 'a4', sender: 'doctor', text: 'Mild drowsiness is expected and should improve in a few days. Continue with the current dose. If drowsiness persists beyond a week, we can adjust. Keep me updated!', time: '4:10 PM', date: 'Feb 20', read: false },
];
  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="care" elevation="sm" padding="lg" style={{ marginBottom: spacing.lg }}>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box display="flex" alignItems="center" style={{ gap: spacing.sm }}>
            ...
            >
              <Text fontSize="sm" fontWeight="bold" color={colors.neutral.white}>MS</Text>
            </div>
            <div>
              <Text fontWeight="bold" fontSize="md" color={colors.journeys.care.text}>{MOCK_DOCTOR.name}</Text>
              <Text fontSize="sm" color={colors.gray[50]}>{MOCK_DOCTOR.specialty}</Text>
            </div>
          </Box>
      ...
      <Card journey="care" elevation="md" padding="lg" style={{ marginBottom: spacing.lg }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.md, minHeight: '260px' }}>
          {MOCK_MESSAGES.map((msg) => (
            <div
              key={msg.id}
              style={{ alignSelf: msg.sender === 'patient' ? 'flex-end' : 'flex-start', maxWidth: '75%' }}
            >
              <div
                style={{
                  padding: spacing.sm,
                  borderRadius: spacing.xs,
                  backgroundColor: msg.sender === 'patient'
      ...
      <Box display="flex" style={{ gap: spacing.sm }}>
        ...
        >
          <Text fontSize="sm" color={colors.gray[40]}>Type your message...</Text>
        </div>
        <Button
          journey="care"
          onPress={() => {}}
