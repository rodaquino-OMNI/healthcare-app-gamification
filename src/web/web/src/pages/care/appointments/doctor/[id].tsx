...
const MOCK_REVIEWS: Review[] = [
  { id: '1', author: 'Maria L.', rating: 5, date: '15/01/2026', comment: 'Excelente profissional, muito atenciosa e competente.' },
  { id: '2', author: 'Joao P.', rating: 4, date: '08/01/2026', comment: 'Otima consulta, explicou tudo com clareza.' },
  { id: '3', author: 'Ana C.', rating: 5, date: '28/12/2025', comment: 'Recomendo muito! Profissional dedicada e cuidadosa.' },
];
          ...
          <Box padding="lg">
            <div style={{ display: 'flex', gap: spacing.lg, alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <Text fontSize="md" color={colors.journeys.care.primary} style={{ marginTop: spacing.xs }}>
                  {doctor.specialty}
                </Text>
        ...
        <Card journey="care" elevation="sm" style={{ marginTop: spacing.md }}>
          <Box padding="md">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm }}>
              <div>
                <Text fontSize="xs" color={colors.gray[50]}>Formacao</Text>
                <Text fontSize="sm" color={colors.journeys.care.text}>{doctor.education}</Text>
              </div>
              <div>
                <Text fontSize="xs" color={colors.gray[50]}>Idiomas</Text>
                <Text fontSize="sm" color={colors.journeys.care.text}>{doctor.languages.join(', ')}</Text>
              </div>
              <div>
                <Text fontSize="xs" color={colors.gray[50]}>Valor da consulta</Text>
                <Text fontSize="sm" fontWeight="bold" color={colors.journeys.care.primary}>{doctor.consultationPrice}</Text>
              </div>
              <div>
                <Text fontSize="xs" color={colors.gray[50]}>Endereco</Text>
                <Text fontSize="sm" color={colors.journeys.care.text}>{doctor.address}</Text>
              </div>
            </div>
          </Box>
        </Card>
        ...
        <Card journey="care" elevation="sm" style={{ marginTop: spacing.md }}>
          <Box padding="md">
            ...
            {MOCK_REVIEWS.map((review) => (
              <div key={review.id} style={{ paddingBottom: spacing.sm, marginBottom: spacing.sm, borderBottom: `1px solid ${colors.neutral.gray300}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Text fontSize="sm" fontWeight="bold" color={colors.journeys.care.text}>{review.author}</Text>
                  <Text fontSize="xs" color={colors.gray[50]}>{review.date}</Text>
                </div>
