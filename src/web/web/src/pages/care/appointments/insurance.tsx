      ...
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: spacing.xl }}>
            ...
            return (
              ...
              >
                <input
                  ...
                  value={plan.id}
                  checked={isSelected}
                  onChange={() => { setSelectedPlan(plan.id); setVerified(false); }}
                  aria-label={plan.name}
                  style={{ accentColor: colors.journeys.care.primary }}
                />
                <div>
                  <Text fontWeight="bold" fontSize="md" color={colors.journeys.care.text}>{plan.name}</Text>
                  {plan.number && <Text fontSize="sm" color={colors.gray[50]}>{plan.number}</Text>}
                </div>
              </label>
            );
          })}
        ...
        {selectedPlan && !verified && (
          <div style={{ textAlign: 'center', marginBottom: spacing.xl }}>
            <Button journey="care" variant="outlined" onPress={handleVerify} accessibilityLabel="Verificar cobertura">
              Verificar Cobertura
            </Button>
          </div>
        )}
            ...
            <Box padding="lg">
              <div style={{ display: 'flex', alignItems: 'center', gap: spacing.sm, marginBottom: spacing.md }}>
                <span style={{
                  ...
                  borderRadius: '12px',
                  backgroundColor: isCovered ? colors.semantic.successBg : colors.semantic.warningBg,
              ...
              <div style={{ display: 'grid', gap: spacing.sm }}>
                <div>
                  <Text fontSize="sm" color={colors.gray[50]}>Copagamento</Text>
                  <Text fontWeight="bold" color={colors.journeys.care.text}>{isCovered ? 'R$ 50,00' : 'R$ 350,00'}</Text>
                </div>
                <div>
                  <Text fontSize="sm" color={colors.gray[50]}>Cobertura</Text>
                  <Text fontWeight="bold" color={colors.journeys.care.text}>{isCovered ? '85%' : '0%'}</Text>
                </div>
                <div>
                  <Text fontSize="sm" color={colors.gray[50]}>Autorizacao</Text>
                  <Text fontWeight="bold" color={colors.journeys.care.text}>{isCovered ? 'Nao necessaria' : 'N/A'}</Text>
                </div>
              </div>
            </Box>
          </Card>
