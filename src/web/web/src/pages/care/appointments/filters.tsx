...
const SPECIALTIES = [
  'Todas', 'Clinico Geral', 'Cardiologia', 'Dermatologia',
  'Ortopedia', 'Pediatria', 'Ginecologia', 'Neurologia', 'Psiquiatria',
];
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
              ...
              {SPECIALTIES.map((spec) => (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: filters.specialty === spec ? colors.journeys.care.background : 'transparent',
                    color: filters.specialty === spec ? colors.journeys.care.primary : colors.journeys.care.text,
                    cursor: 'pointer',
                    fontSize: '14px',
                  }}
              ...
              {DISTANCE_OPTIONS.map((dist) => (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: filters.maxDistance === dist ? colors.journeys.care.background : 'transparent',
                    color: filters.maxDistance === dist ? colors.journeys.care.primary : colors.journeys.care.text,
                    cursor: 'pointer',
                    fontSize: '14px',
                  }}
              ...
              {[0, 3, 4, 4.5].map((rating) => (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: filters.minRating === rating ? colors.journeys.care.background : 'transparent',
                    color: filters.minRating === rating ? colors.journeys.care.primary : colors.journeys.care.text,
                    cursor: 'pointer',
                    fontSize: '14px',
                  }}
          ...
          <Box padding="md">
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: spacing.xs }}>
              {AVAILABILITY_OPTIONS.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => setFilters((prev) => ({ ...prev, availability: opt.value as FilterState['availability'] }))}
                  style={{
                    padding: `${spacing.xs} ${spacing.sm}`,
                    borderRadius: '20px',
                    border: `1px solid ${filters.availability === opt.value ? colors.journeys.care.primary : colors.neutral.gray300}`,
                    backgroundColor: filters.availability === opt.value ? colors.journeys.care.background : 'transparent',
                    color: filters.availability === opt.value ? colors.journeys.care.primary : colors.journeys.care.text,
                    cursor: 'pointer',
                    fontSize: '14px',
                  }}
        ...
        <Card journey="care" elevation="sm" style={{ marginTop: spacing.md }}>
          <Box padding="md" display="flex" justifyContent="space-between" alignItems="center">
            ...
            <button
              onClick={() => setFilters((prev) => ({ ...prev, acceptsInsurance: !prev.acceptsInsurance }))}
              style={{
                width: '48px',
                height: '28px',
                borderRadius: '14px',
                border: 'none',
                backgroundColor: filters.acceptsInsurance ? colors.journeys.care.primary : colors.neutral.gray300,
                cursor: 'pointer',
                position: 'relative',
              }}
              role="switch"
