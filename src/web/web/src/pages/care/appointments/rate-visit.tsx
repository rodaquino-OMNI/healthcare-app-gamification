    ...
    <CareLayout>
      ...
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: spacing.xl }}>
        <Card journey="care" elevation="sm" style={{ marginBottom: spacing.xl }}>
          <Box padding="md" style={{ textAlign: 'center' }}>
            <div style={{
              ...
              <Text fontSize="xl" fontWeight="bold" color={colors.journeys.care.primary}>AS</Text>
            </div>
            <Text fontWeight="bold" fontSize="md" color={colors.journeys.care.text}>Dra. Ana Silva</Text>
            <Text fontSize="sm" color={colors.gray[50]}>Cardiologia — 03/03/2026</Text>
          </Box>
        </Card>
        ...
        <div style={{ display: 'grid', gap: spacing.md, marginBottom: spacing.xl }}>
          {CATEGORIES.map((cat) => (
            <div key={cat.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Text fontSize="sm" color={colors.journeys.care.text}>{cat.label}</Text>
              <StarSelector value={categoryRatings[cat.id]} onChange={(v) => handleCategoryChange(cat.id, v)} label={cat.label} />
            </div>
          ))}
        </div>
          ...
          {[true, false].map((val) => (
            <button
              ...
              style={{
                ...
                backgroundColor: recommend === val ? colors.journeys.care.background : colors.neutral.white,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <Button journey="care" variant="outlined" onPress={() => router.push('/care/appointments/list')} accessibilityLabel="Pular">
            Pular
          </Button>
          <Button journey="care" onPress={handleSubmit} disabled={overallRating === 0} accessibilityLabel="Enviar avaliacao">
            Enviar Avaliacao
          </Button>
        </div>
      </div>
