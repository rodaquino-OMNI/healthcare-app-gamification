  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="care" elevation="sm" padding="lg" style={{ marginBottom: spacing.lg }}>
        <Text fontWeight="medium" fontSize="md" color={colors.journeys.care.text} style={{ marginBottom: spacing.xs }}>
          Doctor Recommendation
        </Text>
        <Text fontSize="sm" color={colors.gray[60]}>
          A follow-up is recommended to evaluate headache progression and assess
          response to prescribed treatment. If symptoms worsen before your follow-up,
        ...
        {MOCK_SUGGESTIONS.map((slot) => (
          ...
          >
            <Box display="flex" justifyContent="space-between" alignItems="center" style={{ marginBottom: spacing.sm }}>
              <div>
                <Text fontWeight="bold" fontSize="md" color={colors.journeys.care.text}>{slot.date}</Text>
                <Text fontSize="sm" color={colors.gray[50]}>{slot.time}</Text>
              </div>
              <div style={{ display: 'flex', gap: spacing.xs }}>
                ...
                {slot.recommended && (
                  <Badge variant="status" status="warning">Recommended</Badge>
                )}
              </div>
            </Box>
            <Button
