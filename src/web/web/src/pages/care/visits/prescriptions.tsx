        ...
        {MOCK_PRESCRIPTIONS.map((rx) => (
          <Card key={rx.id} journey="care" elevation="md" padding="lg">
            <Box display="flex" justifyContent="space-between" alignItems="center" style={{ marginBottom: spacing.sm }}>
              <Text fontWeight="bold" fontSize="lg" color={colors.journeys.care.text}>{rx.name}</Text>
              <Badge variant="status" status="info">{rx.dosage}</Badge>
            </Box>

            <div style={{ display: 'flex', flexDirection: 'column', gap: spacing['3xs'], marginBottom: spacing.sm }}>
              <Box display="flex" justifyContent="space-between">
                <Text fontSize="sm" color={colors.gray[50]}>Frequency</Text>
                <Text fontSize="sm" fontWeight="medium" color={colors.journeys.care.text}>{rx.frequency}</Text>
              </Box>
              <Box display="flex" justifyContent="space-between">
                <Text fontSize="sm" color={colors.gray[50]}>Duration</Text>
                <Text fontSize="sm" fontWeight="medium" color={colors.journeys.care.text}>{rx.duration}</Text>
              </Box>
              <Box display="flex" justifyContent="space-between">
                <Text fontSize="sm" color={colors.gray[50]}>Refills</Text>
                <Text fontSize="sm" fontWeight="medium" color={colors.journeys.care.text}>{rx.refills}</Text>
              </Box>
            </div>
