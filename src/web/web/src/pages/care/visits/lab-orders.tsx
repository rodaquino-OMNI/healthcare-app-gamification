...
const MOCK_ORDERS: LabOrder[] = [
  { id: 'lab1', name: 'Complete Blood Count (CBC)', description: 'Evaluate overall health and detect disorders', urgency: 'routine', fasting: false },
  { id: 'lab2', name: 'Comprehensive Metabolic Panel', description: 'Check kidney function, blood sugar, and electrolytes', urgency: 'urgent', fasting: true },
  { id: 'lab3', name: 'Thyroid Function (TSH)', description: 'Screen for thyroid disorders that may cause headaches', urgency: 'routine', fasting: false },
];

const MOCK_LABS: NearbyLab[] = [
  { id: 'n1', name: 'LabCorp - Downtown', address: '123 Main St, Suite 200', distance: '0.8 km', hours: '7:00 AM - 6:00 PM' },
  { id: 'n2', name: 'Quest Diagnostics - Medical Center', address: '456 Health Ave', distance: '1.2 km', hours: '6:30 AM - 5:00 PM' },
];
...
const LabOrdersPage: React.FC = () => {
  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
        ...
        {MOCK_ORDERS.map((order) => (
          <Card key={order.id} journey="care" elevation="md" padding="lg">
            <Box display="flex" justifyContent="space-between" alignItems="center" style={{ marginBottom: spacing.xs }}>
              <Text fontWeight="bold" fontSize="md" color={colors.journeys.care.text}>{order.name}</Text>
      ...
      <Text fontWeight="bold" fontSize="lg" color={colors.journeys.care.text} style={{ marginBottom: spacing.md }}>
        Nearby Labs
      </Text>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.md, marginBottom: spacing.xl }}>
        {MOCK_LABS.map((lab) => (
          <Card key={lab.id} journey="care" elevation="sm" padding="lg">
            <Box display="flex" justifyContent="space-between" alignItems="center" style={{ marginBottom: spacing.xs }}>
              <Text fontWeight="bold" fontSize="md" color={colors.journeys.care.text}>{lab.name}</Text>
              <Text fontSize="sm" color={colors.gray[50]}>{lab.distance}</Text>
            </Box>
            <Text fontSize="sm" color={colors.gray[60]}>{lab.address}</Text>
            <Text fontSize="sm" color={colors.gray[40]} style={{ marginTop: spacing['3xs'] }}>{lab.hours}</Text>
            <Button
              variant="secondary"
              journey="care"
              onPress={() => {}}
