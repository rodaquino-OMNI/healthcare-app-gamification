      ...
      {showForm ? (
        <div style={{ ...cardStyle, marginTop: spacing.md }}>
          ...
          <div style={fieldGroup}>
            ...
            <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} style={inputStyle} placeholder="Nome completo" />
          </div>
          <div style={fieldGroup}>
            <label style={labelStyle}>Telefone</label>
            <input type="tel" value={newPhone} onChange={(e) => setNewPhone(e.target.value)} style={inputStyle} placeholder="(00) 00000-0000" />
          </div>
          <div style={fieldGroup}>
            <label style={labelStyle}>Parentesco</label>
            <input type="text" value={newRelationship} onChange={(e) => setNewRelationship(e.target.value)} style={inputStyle} placeholder="Ex: Esposo, Mae, Amigo" />
          </div>
          <div style={{ display: 'flex', gap: spacing.xs }}>
            <button onClick={handleAdd} style={saveBtnStyle}>Adicionar</button>
            <button onClick={() => setShowForm(false)} style={cancelBtnStyle}>Cancelar</button>
          </div>
        </div>
      ) : (
        <button onClick={() => setShowForm(true)} style={addBtnStyle}>+ Adicionar Contato</button>
      )}
    </div>
  );
};
...
const phoneStyle: React.CSSProperties = {
  fontSize: typography.fontSize['text-sm'], color: colors.gray[60],
  marginTop: spacing.xs, fontFamily: typography.fontFamily.body,
};
const actionBtnStyle: React.CSSProperties = {
  padding: `${spacing.xs} ${spacing.sm}`, backgroundColor: colors.gray[10],
  color: colors.gray[60], border: 'none', borderRadius: borderRadius.md,
  cursor: 'pointer', fontSize: typography.fontSize['text-xs'], fontFamily: typography.fontFamily.body,
};
const removeBtnStyle: React.CSSProperties = {
  padding: `${spacing.xs} ${spacing.sm}`, backgroundColor: 'transparent',
  color: colors.semantic.error, border: `1px solid ${colors.semantic.error}`,
  borderRadius: borderRadius.md, cursor: 'pointer', fontSize: typography.fontSize['text-xs'],
...
const saveBtnStyle: React.CSSProperties = {
  flex: 1, padding: spacing.sm, backgroundColor: colors.brand.primary,
  color: colors.neutral.white, border: 'none', borderRadius: borderRadius.md,
  cursor: 'pointer', fontSize: typography.fontSize['text-sm'], fontWeight: typography.fontWeight.semiBold,
  fontFamily: typography.fontFamily.body,
};
const cancelBtnStyle: React.CSSProperties = {
  flex: 1, padding: spacing.sm, backgroundColor: 'transparent',
