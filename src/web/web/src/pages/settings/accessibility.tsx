...
const AccessibilityPage: NextPage = () => {
  ...
  const toggles = [
    { label: 'Alto Contraste', desc: 'Aumentar contraste de cores para melhor visibilidade', value: highContrast, toggle: () => setHighContrast(!highContrast) },
    { label: 'Reducao de Movimento', desc: 'Reduzir animacoes e transicoes', value: reducedMotion, toggle: () => setReducedMotion(!reducedMotion) },
        ...
        <div style={{ display: 'flex', gap: spacing.xs }}>
          {fontSizes.map((fs) => (
            <button
              ...
              style={{
                flex: 1, padding: spacing.md, textAlign: 'center' as const,
            ...
            >
              <span style={{ display: 'block', fontSize: fs.preview, fontWeight: typography.fontWeight.semiBold }}>
                Aa
              </span>
              <span style={{ display: 'block', fontSize: typography.fontSize['text-xs'], marginTop: spacing['3xs'], color: colors.gray[50] }}>
                {fs.label}
              </span>
            </button>
          ))}
