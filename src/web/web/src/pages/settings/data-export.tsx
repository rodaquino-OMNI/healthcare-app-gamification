...
const DataExportPage: NextPage = () => {
  const [categories, setCategories] = useState<ExportCategory[]>([
    ...
    { key: 'activity', label: 'Dados de Atividade', description: 'Historico de acesso, dispositivos', selected: false },
    { key: 'gamification', label: 'Dados de Gamificacao', description: 'Conquistas, recompensas, XP', selected: false },
  ]);
    ...
    <div style={{ padding: spacing.xl, maxWidth: '600px', margin: '0 auto' }}>
      ...
      {requested ? (
        <div style={successCardStyle}>
          <h2 style={{ fontSize: typography.fontSize['heading-md'], color: colors.semantic.success, marginBottom: spacing.xs }}>
            Solicitacao Enviada
          </h2>
          ...
          <div style={cardStyle}>
            ...
            {categories.map((cat) => (
              <label key={cat.key} style={checkRowStyle}>
                ...
                />
                <div>
                  <span style={{ fontSize: typography.fontSize['text-md'], color: colors.gray[70] }}>{cat.label}</span>
                  <span style={{ display: 'block', fontSize: typography.fontSize['text-xs'], color: colors.gray[40] }}>{cat.description}</span>
                </div>
              </label>
            ))}
          </div>
            ...
            {(['json', 'csv', 'pdf'] as const).map((f) => (
              <label key={f} style={radioRowStyle}>
                <input
                  ...
                  style={{ marginRight: spacing.sm, accentColor: colors.brand.primary }}
                />
                <span style={{ fontSize: typography.fontSize['text-md'], color: colors.gray[70], textTransform: 'uppercase' }}>{f}</span>
              </label>
            ))}
          </div>
...
const successCardStyle: React.CSSProperties = {
  ...cardStyle, borderLeft: `4px solid ${colors.semantic.success}`,
};
...
const checkRowStyle: React.CSSProperties = {
  display: 'flex', alignItems: 'flex-start', padding: `${spacing.sm} 0`,
  borderBottom: `1px solid ${colors.gray[10]}`, cursor: 'pointer',
};
const radioRowStyle: React.CSSProperties = {
  display: 'flex', alignItems: 'center', padding: `${spacing.xs} 0`, cursor: 'pointer',
};
