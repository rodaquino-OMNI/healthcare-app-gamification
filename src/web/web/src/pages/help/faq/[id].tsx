...
const ARTICLES: Record<string, FaqArticle> = {
  '1': {
    id: '1', question: 'Como alterar minha senha?',
    ...
    category: 'Conta e Acesso', relatedIds: ['2'], lastUpdated: '15/01/2024',
  },
  '2': {
    id: '2', question: 'Como ativar a autenticacao em duas etapas?',
    ...
    category: 'Conta e Acesso', relatedIds: ['1'], lastUpdated: '10/01/2024',
  },
};

const DEFAULT_ARTICLE: FaqArticle = {
  id: '0', question: 'Artigo nao encontrado',
  answer: 'O artigo que voce esta procurando nao foi encontrado. Tente buscar na Central de Ajuda.',
  category: 'Geral', relatedIds: [], lastUpdated: '',
};
  ...
  return (
    <div style={{ padding: spacing.xl, maxWidth: '720px', margin: '0 auto' }}>
      ...
      <div style={cardStyle}>
        ...
        {article.lastUpdated && (
          <p style={lastUpdatedStyle}>Atualizado em {article.lastUpdated}</p>
        )}
      </div>
      ...
      <div style={feedbackCardStyle}>
        ...
        {helpful === null ? (
          <div style={{ display: 'flex', gap: spacing.xs, justifyContent: 'center' }}>
            <button onClick={() => setHelpful(true)} style={feedbackBtnStyle}>Sim</button>
            <button onClick={() => setHelpful(false)} style={feedbackBtnStyle}>Nao</button>
          </div>
        ) : (
          <p style={{ fontSize: typography.fontSize['text-sm'], color: colors.gray[50], textAlign: 'center' as const }}>
...
const categoryBadgeStyle: React.CSSProperties = {
  ...
  color: colors.brand.primary, backgroundColor: colors.brandPalette[50],
  padding: `${spacing['3xs']} ${spacing.xs}`, borderRadius: borderRadius.full,
};
const titleStyle: React.CSSProperties = {
  fontSize: typography.fontSize['heading-lg'], fontWeight: typography.fontWeight.semiBold,
  color: colors.gray[70], marginTop: spacing.md, fontFamily: typography.fontFamily.heading,
};
const paragraphStyle: React.CSSProperties = {
  fontSize: typography.fontSize['text-md'], color: colors.gray[60],
  lineHeight: typography.lineHeight.base, fontFamily: typography.fontFamily.body,
  marginBottom: spacing.md, whiteSpace: 'pre-line',
};
...
const feedbackCardStyle: React.CSSProperties = {
  backgroundColor: colors.gray[5], borderRadius: borderRadius.md,
  padding: spacing.lg, marginTop: spacing.xl, textAlign: 'center' as const,
};
const feedbackQuestionStyle: React.CSSProperties = {
  ...
  color: colors.gray[60], marginBottom: spacing.sm, fontFamily: typography.fontFamily.body,
};
const feedbackBtnStyle: React.CSSProperties = {
  padding: `${spacing.xs} ${spacing.xl}`, backgroundColor: colors.gray[0],
  color: colors.gray[60], border: `1px solid ${colors.gray[20]}`,
...
const relatedLinkStyle: React.CSSProperties = {
  ...
  cursor: 'pointer', padding: `${spacing.xs} 0`, fontFamily: typography.fontFamily.body,
};
