    ...
    <div style={{ padding: spacing.xl, maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div style={cardStyle}>
        ...
        <div style={fieldGroup}>
          ...
          <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} style={inputStyle} placeholder="Resumo do problema" />
        </div>
          ...
          <div style={{ display: 'flex', gap: spacing.xs, flexWrap: 'wrap' as const }}>
            {severities.map((sev) => (
              ...
              >
                ...
                <span style={{ fontSize: typography.fontSize['text-2xs'], color: colors.gray[40] }}>{sev.desc}</span>
              </button>
            ))}
          </div>
        </div>
...
const severityBtnStyle: React.CSSProperties = {
  flex: '1 1 calc(50% - 4px)', padding: spacing.xs, borderRadius: borderRadius.md,
  border: `2px solid ${colors.gray[20]}`, cursor: 'pointer',
  display: 'flex', flexDirection: 'column', alignItems: 'center', gap: spacing['3xs'],
  fontSize: typography.fontSize['text-sm'], color: colors.gray[70],
  fontFamily: typography.fontFamily.body, backgroundColor: colors.gray[0],
};
