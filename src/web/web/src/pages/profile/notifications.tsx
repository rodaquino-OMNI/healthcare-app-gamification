...
export default function NotificationPreferencesPage() {
  ...
  const essentialPrefs = preferences.filter(p =>
    ['push', 'email'].includes(p.key)
  );
  const healthPrefs = preferences.filter(p =>
    ['medications', 'appointments', 'health_goals'].includes(p.key)
  );
  const otherPrefs = preferences.filter(p =>
    ['plan_updates', 'achievements', 'marketing'].includes(p.key)
  );
