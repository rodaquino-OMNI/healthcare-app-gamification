  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.lg }}>
        <Text fontSize="sm" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>Food Name</Text>
        <input type="text" value={foodName} onChange={(e) => setFoodName(e.target.value)} placeholder="e.g. Grilled chicken breast" aria-label="Food name" style={{ ...inputStyle, marginBottom: spacing.md }} />
        <Text fontSize="sm" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>Portion Size</Text>
        <input type="text" value={portion} onChange={(e) => setPortion(e.target.value)} placeholder="e.g. 150g or 1 cup" aria-label="Portion size" style={inputStyle} />
      </Card>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm, marginBottom: spacing['2xl'] }}>
        {MOCK_MACROS.map((macro) => (
          <Card key={macro.label} journey="health" elevation="sm" padding="md">
            <Text fontSize="xs" color={colors.gray[50]}>{macro.label}</Text>
            <Text fontSize="lg" fontWeight="bold" color={colors.journeys.health.primary}>{macro.value}</Text>
          </Card>
        ))}
      </div>
