...
const MealDetailPage: React.FC = () => {
  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Box display="flex" alignItems="center" style={{ gap: spacing.sm, marginTop: spacing.xs, marginBottom: spacing.xl }}>
        <Text fontSize="md" color={colors.gray[50]}>Meal #{id || '—'}</Text>
        <div style={{ padding: `2px ${spacing.xs}`, borderRadius: '12px', backgroundColor: `${colors.journeys.health.primary}22` }}>
          <Text fontSize="xs" fontWeight="semiBold" color={colors.journeys.health.primary}>Lunch</Text>
        </div>
      </Box>

      <Card journey="health" elevation="sm" padding="lg" style={{ marginBottom: spacing.xl, textAlign: 'center' }}>
        <Text fontSize="sm" color={colors.gray[50]}>Grilled Chicken Bowl</Text>
        <Text fontSize="2xl" fontWeight="bold" color={colors.journeys.health.primary} style={{ margin: `${spacing.xs} 0` }}>
          420 kcal
        </Text>
        <Text fontSize="sm" color={colors.gray[40]}>Feb 23, 2026 at 12:30 PM</Text>
      </Card>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: spacing.sm, marginBottom: spacing.xl }}>
        {NUTRIENTS.map((nutrient) => (
          <Card key={nutrient.label} journey="health" elevation="sm" padding="md" style={{ textAlign: 'center' }}>
            <Text fontSize="xs" color={colors.gray[50]}>{nutrient.label}</Text>
            <Text fontSize="md" fontWeight="bold" color={colors.journeys.health.primary}>{nutrient.value}</Text>
          </Card>
        ))}
      </div>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing['2xl'] }}>
        {INGREDIENTS.map((item, idx) => (
          <div key={item}>
            <Text fontSize="sm" color={colors.gray[60]}>{item}</Text>
      ...
      <Box display="flex" style={{ gap: spacing.sm }}>
        ...
        <Box style={{ flex: 1 }}>
          <Button variant="secondary" journey="health" onPress={handleDelete} accessibilityLabel="Delete meal">
            Delete
          </Button>
        </Box>
      </Box>
