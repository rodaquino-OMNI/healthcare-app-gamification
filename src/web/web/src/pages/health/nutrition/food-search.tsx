...
const FoodSearchPage: React.FC = () => {
  ...
  const results = searched ? MOCK_RESULTS.filter((r) => r.name.toLowerCase().includes(query.toLowerCase()) || query === '') : [];
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: spacing.xs, marginBottom: spacing.xl }}>
            {RECENT_SEARCHES.map((term) => (
              <div
                ...
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { setQuery(term); setSearched(true); } }}
                style={{ padding: `${spacing['3xs']} ${spacing.xs}`, borderRadius: '16px', backgroundColor: `${colors.journeys.health.primary}22`, cursor: 'pointer' }}
              >
                <Text fontSize="sm" color={colors.journeys.health.primary}>{term}</Text>
              </div>
            ))}
          </div>
        </>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm }}>
            {results.length === 0 ? (
              <Card journey="health" elevation="sm" padding="md">
                <Text fontSize="md" color={colors.gray[50]}>No results found. Try a different search term.</Text>
              </Card>
            ) : (
              results.map((food) => (
                <Card key={food.id} journey="health" elevation="sm" padding="md">
                  <Box display="flex" justifyContent="space-between" alignItems="center">
                    <Box>
                      <Text fontSize="md" fontWeight="semiBold" color={colors.journeys.health.text}>{food.name}</Text>
                      <Text fontSize="sm" color={colors.gray[50]}>{food.calories} kcal / {food.per}</Text>
                    </Box>
                    <Button variant="secondary" journey="health" onPress={() => handleAdd(food.name)} accessibilityLabel={`Add ${food.name}`}>
                      Add
                    </Button>
                  </Box>
                </Card>
