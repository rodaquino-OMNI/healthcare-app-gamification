    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm, marginBottom: spacing['2xl'] }}>
        {QUICK_STATS.map((stat) => (
          <Card key={stat.label} journey="health" elevation="sm" padding="md">
            <Text fontSize="sm" color={colors.gray[50]}>{stat.label}</Text>
            <Text fontSize="xl" fontWeight="bold" color={colors.journeys.health.primary}>{stat.value}</Text>
          </Card>
        ))}
      </div>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing['2xl'] }}>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box>
            <Text fontSize="md" fontWeight="semiBold" color={colors.journeys.health.text}>Today&apos;s Goal</Text>
            <Text fontSize="sm" color={colors.gray[50]}>2,000 kcal target</Text>
          </Box>
          <Text fontSize="lg" fontWeight="bold" color={colors.semantic.success}>92%</Text>
        </Box>
      </Card>
