  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.xl }}>
        <Text fontSize="sm" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>Daily Calories (kcal)</Text>
        <input
          type="number"
          value={calorieTarget}
          ...
          style={inputStyle}
        />
        <Text fontSize="xs" color={colors.gray[40]} style={{ marginTop: spacing['3xs'] }}>Recommended: 1,800 – 2,200 kcal for adults</Text>
      </Card>
        ...
        {MACRO_SPLITS.map((macro) => (
          <Card key={macro.label} journey="health" elevation="sm" padding="md">
            <Box display="flex" justifyContent="space-between" alignItems="center" style={{ marginBottom: spacing.xs }}>
              <Text fontSize="md" color={colors.journeys.health.text}>{macro.label}</Text>
              <Text fontSize="md" fontWeight="bold" color={macro.color}>{macro.pct}%</Text>
            </Box>
            <div style={{ height: 8, borderRadius: '4px', backgroundColor: colors.gray[10], overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${macro.pct}%`, borderRadius: '4px', backgroundColor: macro.color }} />
            </div>
            <Text fontSize="xs" color={colors.gray[40]} style={{ marginTop: spacing['3xs'] }}>
              {Math.round((parseInt(calorieTarget || '2000') * macro.pct) / 100 / (macro.label === 'Fat' ? 9 : 4))}g / day
            </Text>
          </Card>
        ))}
      </div>
      ...
      <Box display="flex" style={{ gap: spacing.sm }}>
        ...
        <Box style={{ flex: 1 }}>
          <Button variant="secondary" journey="health" onPress={handleReset} accessibilityLabel="Reset to defaults">
            Reset
          </Button>
        </Box>
      </Box>
