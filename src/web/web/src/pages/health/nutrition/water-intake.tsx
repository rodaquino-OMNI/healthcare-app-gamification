...
const WaterIntakePage: React.FC = () => {
  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="lg" style={{ textAlign: 'center', marginBottom: spacing.xl }}>
        <Text fontSize="sm" color={colors.gray[50]}>Today's Hydration</Text>
        ...
        <div style={{ height: 12, borderRadius: '6px', backgroundColor: colors.gray[10], overflow: 'hidden', margin: `${spacing.xs} 0` }}>
          <div style={{ height: '100%', width: `${pct}%`, borderRadius: '6px', backgroundColor: colors.journeys.health.primary }} />
        </div>
        <Text fontSize="sm" color={colors.gray[40]}>{pct}% of daily goal</Text>
      </Card>
      ...
      <Box display="flex" style={{ gap: spacing.sm, marginBottom: spacing.xl }}>
        {[1, 2, 3].map((amount) => (
          <Button key={amount} variant="secondary" journey="health" onPress={() => handleAdd(amount)} accessibilityLabel={`Add ${amount} glass`}>
        ...
        {HISTORY.map((entry, idx) => (
          <div key={entry.time}>
            <Box display="flex" justifyContent="space-between" alignItems="center">
              <Text fontSize="sm" color={colors.gray[60]}>{entry.time}</Text>
              <Text fontSize="sm" fontWeight="semiBold" color={colors.journeys.health.primary}>{entry.amount}</Text>
            </Box>
      ...
      <Card journey="health" elevation="sm" padding="md">
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Text fontSize="md" fontWeight="semiBold" color={colors.journeys.health.text}>Hydration Streak</Text>
          <Text fontSize="xl" fontWeight="bold" color={colors.journeys.health.primary}>9 days</Text>
        </Box>
      </Card>
    </div>
  );
