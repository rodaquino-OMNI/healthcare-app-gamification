...
const buildMockSchedule = (days: CalendarDay[]): Record<string, DoseSlot[]> => {
  ...
  days.forEach((day, idx) => {
    schedule[day.fullDate] = [
      { id: `${day.fullDate}-1`, medicationName: 'Metformin', dosage: '500mg', time: '08:00 AM', status: idx < 4 ? 'taken' : idx === 4 ? 'missed' : 'pending' },
      { id: `${day.fullDate}-2`, medicationName: 'Lisinopril', dosage: '10mg', time: '08:00 AM', status: idx < 5 ? 'taken' : 'pending' },
      { id: `${day.fullDate}-3`, medicationName: 'Metformin', dosage: '500mg', time: '02:00 PM', status: idx < 3 ? 'taken' : 'pending' },
      { id: `${day.fullDate}-4`, medicationName: 'Atorvastatin', dosage: '20mg', time: '08:00 PM', status: idx < 2 ? 'taken' : idx === 2 ? 'missed' : 'pending' },
    ];
  });
  return schedule;
};
          ...
          return (
            <button
              ...
              style={{
                ...
                height: 72,
                borderRadius: 12,
                border: isSelected ? `2px solid ${colors.journeys.health.primary}` : `1px solid ${colors.neutral.gray300}`,
              ...
              <Card key={dose.id} journey="health" elevation="sm" padding="md">
                <Box display="flex" justifyContent="space-between" alignItems="center">
                  <Box display="flex" alignItems="center" style={{ gap: spacing.md }}>
                    <Text fontSize="sm" fontWeight="semiBold" color={colors.journeys.health.primary} style={{ width: 80 }}>
                      {dose.time}
                    </Text>
                    <Box>
                      <Text fontWeight="medium" fontSize="md">{dose.medicationName}</Text>
                      <Text fontSize="sm" color={colors.gray[50]}>{dose.dosage}</Text>
                    </Box>
                  </Box>
                  <Badge variant="status" status={config.badge}>{config.label}</Badge>
                </Box>
              </Card>
            );
          })}
