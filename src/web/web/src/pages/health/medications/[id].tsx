    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.md }}>
          <Box>
            <Text fontSize="xs" color={colors.gray[40]}>Frequency</Text>
            <Text fontSize="sm">{medication.frequency}</Text>
          </Box>
          <Box>
            <Text fontSize="xs" color={colors.gray[40]}>Start Date</Text>
            <Text fontSize="sm">{medication.startDate}</Text>
          </Box>
          <Box>
            <Text fontSize="xs" color={colors.gray[40]}>Prescribed By</Text>
            <Text fontSize="sm">{medication.prescribedBy}</Text>
          </Box>
          {medication.refillDate && (
            <Box>
              <Text fontSize="xs" color={colors.gray[40]}>Next Refill</Text>
        ...
        {medication.instructions && (
          <Box style={{ marginTop: spacing.md }}>
            <Text fontSize="xs" color={colors.gray[40]}>Instructions</Text>
          ...
          {medication.doseHistory.map((dose, idx) => (
            <Box
              ...
              style={{
                ...
                borderBottom: idx < medication.doseHistory.length - 1
            ...
            >
              <Box>
                ...
                {dose.time && (
                  <Text fontSize="xs" color={colors.gray[40]}>{dose.time}</Text>
                )}
              </Box>
              <Badge
                variant="status"
                status={dose.taken ? 'success' : 'error'}
              >
