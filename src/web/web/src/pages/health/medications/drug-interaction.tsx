...
const MOCK_INTERACTIONS: DrugInteraction[] = [
  {
    ...
    recommendation:
      'Monitor kidney function regularly. Consider acetaminophen as an alternative pain reliever.',
  },
  {
    id: '2',
    drugA: 'Lisinopril',
  ...
  },
  {
    ...
    recommendation:
      'Limit grapefruit juice consumption. One small glass per day is generally safe.',
  },
];
...
const MedicationDrugInteractionPage: React.FC = () => {
  ...
  const highestSeverity: SeverityLevel = MOCK_INTERACTIONS.reduce<SeverityLevel>(
    (highest, interaction) => {
      const order: SeverityLevel[] = ['minor', 'moderate', 'severe'];
      return order.indexOf(interaction.severity) > order.indexOf(highest)
        ...
        return (
          ...
          >
            ...
            <Box display="flex" alignItems="center" style={{ gap: spacing.xs, marginBottom: spacing.xs }}>
              <Text fontWeight="semiBold" fontSize="md">{interaction.drugA}</Text>
              <Text fontSize="sm" color={colors.gray[50]}>+</Text>
              <Text fontWeight="semiBold" fontSize="md">{interaction.drugB}</Text>
            </Box>
