...
const StepHealthGoalsPage: React.FC<StepProps> = ({ data, onUpdate }) => {
  ...
  const toggleGoal = (key: string) => {
    const updated = selectedGoals.includes(key)
      ? selectedGoals.filter((g) => g !== key)
      : [...selectedGoals, key];
    onUpdate('healthGoals', updated);
    if (selectedGoals.includes(key)) {
      onUpdate('goalPriorities', priorities.filter((p) => p !== key));
    }
  };
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isPriority ? colors.journeys.health.primary : colors.neutral.white,
