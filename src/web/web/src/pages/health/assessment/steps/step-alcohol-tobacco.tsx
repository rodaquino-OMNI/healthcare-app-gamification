...
const StepAlcoholTobaccoPage: React.FC<StepProps> = ({ data, onUpdate }) => {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg }}>
      ...
      {(data.smokingStatus === 'former' || data.smokingStatus === 'occasional' || data.smokingStatus === 'daily') && (
        <Card journey="health" elevation="sm" padding="md">
          <Text fontSize="sm" fontWeight="medium" color={colors.gray[60]} style={{ marginBottom: spacing['3xs'] }}>
            {data.smokingStatus === 'former' ? 'How many years did you smoke?' : 'How many years have you smoked?'}
          </Text>
          <input
            style={{
              width: '100%',
