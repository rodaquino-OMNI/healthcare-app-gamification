    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg }}>
      ...
      <Card journey="health" elevation="sm" padding="lg">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.xs }}>
          {COVID_OPTIONS.map(({ key, label }) => {
            ...
            return (
              <button key={key} onClick={() => onUpdate('covidStatus', key)} style={optionStyle(isActive)} aria-pressed={isActive} aria-label={label}>
      ...
      <Card journey="health" elevation="sm" padding="lg">
        ...
        <div style={{ display: 'flex', gap: spacing.xs }}>
          {['yes', 'no'].map((opt) => (
            <button key={opt} onClick={() => onUpdate('fluVaccine', opt)} style={chipStyle(data.fluVaccine === opt)} aria-pressed={data.fluVaccine === opt} aria-label={opt === 'yes' ? 'Yes' : 'No'}>
              {opt === 'yes' ? 'Yes' : 'No'}
            </button>
          ))}
        </div>
      ...
      <Card journey="health" elevation="sm" padding="lg">
        ...
        <div style={{ display: 'flex', gap: spacing.xs }}>
          {[
            ...
            <button key={key} onClick={() => onUpdate('otherVaccines', key)} style={chipStyle(data.otherVaccines === key)} aria-pressed={data.otherVaccines === key} aria-label={label}>
              {label}
            </button>
          ))}
        </div>
      ...
      <Card journey="health" elevation="sm" padding="lg">
        ...
        <div style={{ display: 'flex', gap: spacing.xs }}>
          {['yes', 'no'].map((opt) => (
            <button key={opt} onClick={() => onUpdate('vaccinationCard', opt)} style={chipStyle(data.vaccinationCard === opt)} aria-pressed={data.vaccinationCard === opt} aria-label={opt === 'yes' ? 'Yes' : 'No'}>
              {opt === 'yes' ? 'Yes' : 'No'}
            </button>
          ))}
        </div>
      ...
      <Card journey="health" elevation="sm" padding="lg" style={{ borderLeft: `3px solid ${colors.semantic.info}` }}>
        ...
        <Text fontSize="xs" color={colors.gray[50]} style={{ marginTop: spacing['3xs'] }}>
          Keeping your vaccinations up to date is one of the most effective ways to prevent serious diseases. We can help remind you about upcoming vaccinations.
        </Text>
      </Card>
    </div>
  );
