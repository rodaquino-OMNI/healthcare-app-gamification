...
const StepChronicPainPage: React.FC<StepProps> = ({ data, onUpdate }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg }}>
          ...
          <Card journey="health" elevation="sm" padding="lg">
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: spacing.xs }}>
              {LOCATIONS.map((location) => {
                ...
                return (
                  <button key={location} onClick={() => toggleLocation(location)} style={chipStyle(selected)} aria-pressed={selected} aria-label={`${location} ${selected ? 'selected' : 'not selected'}`}>
                    {selected ? '\u2713 ' : ''}{location}
                  </button>
                );
              })}
          ...
          <Card journey="health" elevation="sm" padding="lg">
            ...
            <div style={{ display: 'flex', gap: spacing.xs }}>
              {FREQUENCIES.map(({ key, label }) => {
                ...
                return (
                  <button key={key} onClick={() => onUpdate('painFrequency', key)} style={{ flex: 1, padding: spacing.xs, borderRadius: 8, border: `1px solid ${isActive ? colors.journeys.health.primary : colors.neutral.gray300}`, backgroundColor: isActive ? colors.journeys.health.primary : colors.neutral.white, color: isActive ? colors.neutral.white : colors.neutral.gray900, cursor: 'pointer', fontSize: 13, fontWeight: isActive ? 600 : 400 }} aria-pressed={isActive} aria-label={label}>
                    {label}
                  </button>
                );
              })}
          ...
          <Card journey="health" elevation="sm" padding="lg">
            ...
            <div style={{ display: 'flex', gap: spacing.xs }}>
              {IMPACTS.map(({ key, label }) => {
                ...
                return (
                  <button key={key} onClick={() => onUpdate('painImpact', key)} style={{ flex: 1, padding: spacing.xs, borderRadius: 8, border: `1px solid ${isActive ? colors.journeys.health.primary : colors.neutral.gray300}`, backgroundColor: isActive ? colors.journeys.health.primary : colors.neutral.white, color: isActive ? colors.neutral.white : colors.neutral.gray900, cursor: 'pointer', fontSize: 13, fontWeight: isActive ? 600 : 400 }} aria-pressed={isActive} aria-label={label}>
                    {label}
                  </button>
                );
              })}
      ...
      <Card journey="health" elevation="sm" padding="lg" style={{ borderLeft: `3px solid ${colors.journeys.health.primary}` }}>
        ...
        <Text fontSize="xs" color={colors.gray[50]} style={{ marginTop: spacing['3xs'] }}>
          Chronic pain management often benefits from a multidisciplinary approach. We can help connect you with specialists based on your responses.
        </Text>
      </Card>
    </div>
  );
