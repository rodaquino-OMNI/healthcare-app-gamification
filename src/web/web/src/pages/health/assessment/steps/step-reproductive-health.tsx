    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button
          ...
          style={{ background: 'none', border: 'none', color: colors.journeys.health.primary, cursor: 'pointer', fontWeight: 600, fontSize: 13 }}
      ...
      <Card journey="health" elevation="sm" padding="lg">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.xs }}>
          {PREGNANCY_OPTIONS.map(({ key, label }) => {
            ...
            return (
              <button key={key} onClick={() => onUpdate('pregnancyStatus', key)} style={optionStyle(isActive)} aria-pressed={isActive} aria-label={label}>
                <div style={{ width: 18, height: 18, borderRadius: '50%', border: `2px solid ${isActive ? colors.journeys.health.primary : colors.neutral.gray400}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {isActive && <div style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: colors.journeys.health.primary }} />}
                </div>
                {label}
              </button>
            );
      ...
      <Card journey="health" elevation="sm" padding="lg">
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: spacing.xs }}>
          {CHECKUP_OPTIONS.map(({ key, label }) => {
            ...
            return (
              <button key={key} onClick={() => onUpdate('lastReproductiveCheckup', key)} style={{ padding: `${spacing['3xs']} ${spacing.sm}`, borderRadius: 20, border: `1px solid ${isActive ? colors.journeys.health.primary : colors.neutral.gray300}`, backgroundColor: isActive ? colors.journeys.health.background : colors.neutral.white, cursor: 'pointer', fontSize: 13, fontWeight: isActive ? 600 : 400, color: isActive ? colors.journeys.health.accent : colors.neutral.gray900 }} aria-pressed={isActive} aria-label={label}>
                {isActive ? '\u2713 ' : ''}{label}
              </button>
            );
          })}
      ...
      <Card journey="health" elevation="sm" padding="lg">
        ...
        <div style={{ display: 'flex', gap: spacing.xs }}>
          {CONTRACEPTION_OPTIONS.map(({ key, label }) => (
            <button key={key} onClick={() => onUpdate('contraception', key)} style={chipStyle(data.contraception === key)} aria-pressed={data.contraception === key} aria-label={label}>
              {label}
            </button>
          ))}
        </div>
      ...
      <Card journey="health" elevation="sm" padding="lg">
        ...
        <div style={{ display: 'flex', gap: spacing.xs }}>
          {MENSTRUAL_OPTIONS.map(({ key, label }) => (
            <button key={key} onClick={() => onUpdate('menstrualRegularity', key)} style={chipStyle(data.menstrualRegularity === key)} aria-pressed={data.menstrualRegularity === key} aria-label={label}>
              {label}
            </button>
          ))}
        </div>
      ...
      <Card journey="health" elevation="sm" padding="lg" style={{ borderLeft: `3px solid ${colors.semantic.info}` }}>
