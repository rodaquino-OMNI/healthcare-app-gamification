...
const StepConsentPrivacyPage: React.FC<StepProps> = ({ data, onUpdate }) => {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.md }}>
          {CONSENT_ITEMS.map((item) => {
            ...
            return (
              <button
                key={item.key}
                onClick={() => onUpdate(item.key, !data[item.key])}
                style={{
                  display: 'flex',
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <Text fontSize="sm" fontWeight={checked ? 'semiBold' : 'regular'} color={colors.neutral.gray900}>
                    {item.label}
                  </Text>
                  <Text fontSize="xs" color={colors.neutral.gray600} style={{ marginTop: spacing['3xs'] }}>
                    {item.description}
                  </Text>
                </div>
              </button>
      ...
      {/* Data Usage Info */}
      <Card journey="health" elevation="sm" padding="lg">
        <Text fontSize="sm" fontWeight="semiBold" color={colors.journeys.health.text} style={{ marginBottom: spacing.xs }}>
          How We Use Your Data
        </Text>
        <Text fontSize="sm" color={colors.neutral.gray600} style={{ lineHeight: '20px' }}>
          Your health assessment data is used to generate personalized health insights,
          connect you with the right healthcare professionals, and improve the quality
          of your care plan. We never sell your data to third parties.
        </Text>
      </Card>
