...
const StepAnxietyScalePage: React.FC<StepProps> = ({ data, onUpdate }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg }}>
            ...
            {OPTIONS.map((option) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {isActive && (
                      <div style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: colors.journeys.health.primary }} />
                    )}
                  </div>
                  {option.label}
                </button>
      ...
      {bothAnswered && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: spacing.sm, backgroundColor: colors.neutral.gray100, borderRadius: 8 }}>
          <Text fontSize="sm" color={colors.neutral.gray700}>GAD-2 Score</Text>
          <div style={{ padding: `${spacing['3xs']} ${spacing.sm}`, borderRadius: 20, backgroundColor: score >= 3 ? colors.semantic.warning : colors.journeys.health.primary, color: colors.neutral.white, fontWeight: 700, fontSize: 14 }}>
            {score}/6
          </div>
        </div>
      )}
      ...
      <Card journey="health" elevation="sm" padding="lg">
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: spacing.xs }}>
          {TRIGGERS.map(({ key, label }) => {
            ...
            return (
              <button key={key} onClick={() => toggleTrigger(key)} style={chipStyle(selected)} aria-pressed={selected} aria-label={`${label} ${selected ? 'selected' : 'not selected'}`}>
                {selected ? '\u2713 ' : ''}{label}
              </button>
            );
          })}
      ...
      <Card journey="health" elevation="sm" padding="lg" style={{ borderLeft: `3px solid ${colors.semantic.info}` }}>
        ...
        <Text fontSize="xs" color={colors.gray[50]} style={{ marginTop: spacing['3xs'] }}>
          This screening is not a diagnosis. If you scored 3 or higher, we recommend discussing your results with a healthcare professional.
        </Text>
      </Card>
    </div>
  );
