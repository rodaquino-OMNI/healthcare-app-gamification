  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: spacing.xs }}>
          {WORKOUT_TYPES.map((w) => (
            <div
              ...
              aria-checked={type === w}
              tabIndex={0}
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setType(w); }}
              style={{
                cursor: 'pointer', textAlign: 'center', padding: spacing.xs, borderRadius: '8px',
                backgroundColor: type === w ? colors.journeys.health.primary : colors.gray[5],
              }}
            >
              <Text fontSize="sm" color={type === w ? colors.gray[0] : colors.gray[60]}>{w}</Text>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Box display="flex" style={{ gap: spacing.md, marginBottom: spacing.lg }}>
        <Box style={{ flex: 1 }}>
          <Text fontSize="sm" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>Duration (min)</Text>
          <input type="number" value={duration} onChange={(e) => setDuration(e.target.value)} aria-label="Duration in minutes" style={inputStyle} />
        </Box>
        <Box style={{ flex: 1 }}>
          <Text fontSize="sm" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>Intensity</Text>
          <select value={intensity} onChange={(e) => setIntensity(e.target.value)} aria-label="Intensity level" style={inputStyle}>
