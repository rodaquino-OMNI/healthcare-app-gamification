  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm }}>
        {filtered.map((ex) => (
          <div key={ex.id} onClick={() => router.push('/health/activity/exercise-detail')} role="link" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter') router.push('/health/activity/exercise-detail'); }} style={{ cursor: 'pointer' }}>
            <Card journey="health" elevation="sm" padding="md">
              <div style={{
                width: '100%', height: 80, borderRadius: '8px',
                backgroundColor: colors.gray[10], display: 'flex',
                alignItems: 'center', justifyContent: 'center', marginBottom: spacing.sm,
              }}>
                <Text fontSize="sm" color={colors.gray[40]}>{ex.category}</Text>
              </div>
              <Text fontSize="md" fontWeight="semiBold" color={colors.journeys.health.text}>{ex.name}</Text>
              <div style={{
                display: 'inline-block', marginTop: spacing.xs, padding: `${spacing['3xs']} ${spacing.xs}`,
                borderRadius: '12px', backgroundColor: DIFFICULTY_COLORS[ex.difficulty],
              }}>
                <Text fontSize="xs" color={colors.gray[0]}>{ex.difficulty}</Text>
              </div>
            </Card>
          </div>
        ))}
