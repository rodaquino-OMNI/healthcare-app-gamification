  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm }}>
        {filtered.map((w) => (
          <div key={w.id} onClick={() => router.push(`/health/activity/${w.id}`)} role="link" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter') router.push(`/health/activity/${w.id}`); }} style={{ cursor: 'pointer' }}>
            <Card journey="health" elevation="sm" padding="md">
              <Box display="flex" justifyContent="space-between" alignItems="center">
                <Box>
                  <Text fontSize="md" fontWeight="semiBold" color={colors.journeys.health.text}>{w.type}</Text>
                  <Text fontSize="sm" color={colors.gray[50]}>{w.date}</Text>
                </Box>
                <Box style={{ textAlign: 'right' }}>
                  <Text fontSize="sm" fontWeight="semiBold" color={colors.journeys.health.primary}>{w.duration}</Text>
                  <Text fontSize="xs" color={colors.gray[40]}>{w.calories}</Text>
                </Box>
              </Box>
            </Card>
          </div>
