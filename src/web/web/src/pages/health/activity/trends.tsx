  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Box display="flex" style={{ gap: spacing.sm, marginBottom: spacing.xl }}>
        <Button variant={period === '7d' ? 'primary' : 'secondary'} journey="health" onPress={() => setPeriod('7d')} accessibilityLabel="7 days">7 Days</Button>
        <Button variant={period === '30d' ? 'primary' : 'secondary'} journey="health" onPress={() => setPeriod('30d')} accessibilityLabel="30 days">30 Days</Button>
      </Box>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: spacing.sm, height: 120 }}>
          {WEEKLY_STEPS.map((steps, i) => (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <Text fontSize="xs" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>{(steps / 1000).toFixed(1)}k</Text>
              <div style={{
                width: '100%', height: `${(steps / MAX_STEPS) * 100}%`,
                backgroundColor: colors.journeys.health.primary, borderRadius: '4px 4px 0 0',
                minHeight: 8,
              }} />
              <Text fontSize="xs" color={colors.gray[40]} style={{ marginTop: spacing['3xs'] }}>{WEEKLY_LABELS[i]}</Text>
            </div>
          ))}
        </div>
      </Card>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm, marginBottom: spacing.xl }}>
        {STATS.map((s) => (
          <Card key={s.label} journey="health" elevation="sm" padding="md">
            <Text fontSize="xs" color={colors.gray[50]}>{s.label}</Text>
            <Text fontSize="lg" fontWeight="bold" color={colors.journeys.health.primary}>{s.value}</Text>
          </Card>
        ))}
      </div>
      ...
      <Card journey="health" elevation="sm" padding="md">
        <Box display="flex" justifyContent="space-between" style={{ marginBottom: spacing.sm }}>
          <Text fontSize="sm" color={colors.gray[50]}>This Month</Text>
          <Text fontSize="sm" fontWeight="semiBold" color={colors.journeys.health.primary}>248,600 steps</Text>
        </Box>
        <Box display="flex" justifyContent="space-between">
          <Text fontSize="sm" color={colors.gray[50]}>Last Month</Text>
          <Text fontSize="sm" fontWeight="semiBold" color={colors.gray[60]}>231,400 steps</Text>
        </Box>
        <div style={{ height: 1, backgroundColor: colors.gray[10], margin: `${spacing.sm} 0` }} />
        <Box display="flex" justifyContent="space-between">
          <Text fontSize="sm" color={colors.gray[50]}>Change</Text>
          <Text fontSize="sm" fontWeight="semiBold" color={colors.semantic.success}>+7.4%</Text>
        </Box>
      </Card>
    </div>
  );
