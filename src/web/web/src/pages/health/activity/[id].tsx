  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="lg" style={{ marginBottom: spacing.xl, textAlign: 'center' }}>
        <div style={{
          display: 'inline-block', padding: `${spacing['3xs']} ${spacing.sm}`, borderRadius: '16px',
          backgroundColor: colors.journeys.health.primary,
        }}>
          <Text fontSize="sm" fontWeight="semiBold" color={colors.gray[0]}>Running</Text>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm, marginBottom: spacing.xl }}>
        {STATS.map((stat) => (
          <Card key={stat.label} journey="health" elevation="sm" padding="md">
            <Text fontSize="xs" color={colors.gray[50]}>{stat.label}</Text>
            <Text fontSize="lg" fontWeight="bold" color={colors.journeys.health.primary}>{stat.value}</Text>
          </Card>
        ))}
      </div>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing['2xl'] }}>
        <Text fontSize="sm" color={colors.gray[60]}>
          Morning run in the park. Felt great, maintained a steady pace throughout. Weather was clear and cool.
        </Text>
      </Card>

      <Box display="flex" style={{ gap: spacing.sm }}>
        <Box style={{ flex: 1 }}>
          <Button journey="health" onPress={() => router.push('/health/activity/workout-log')} accessibilityLabel="Log another workout">
            Log Another
          </Button>
        </Box>
        <Box style={{ flex: 1 }}>
          <Button variant="secondary" journey="health" onPress={handleDelete} accessibilityLabel="Delete workout">
            Delete
          </Button>
        </Box>
      </Box>
