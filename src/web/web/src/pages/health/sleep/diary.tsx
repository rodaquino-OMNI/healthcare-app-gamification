  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm }}>
        {filtered.map((entry) => (
          <div key={entry.id} onClick={() => router.push(`/health/sleep/${entry.id}`)} style={{ cursor: 'pointer' }}>
            <Card journey="health" elevation="sm" padding="md">
              <Box display="flex" justifyContent="space-between" alignItems="center">
                <Box>
                  <Text fontWeight="semiBold" fontSize="md">{entry.date}</Text>
                  <Text fontSize="sm" color={colors.gray[50]}>{entry.hours}</Text>
                </Box>
                <Box display="flex" alignItems="center" style={{ gap: spacing.sm }}>
                  ...
                  <div style={{
                    ...
                    <Text fontSize="sm" fontWeight="bold" color={scoreColor(entry.score)}>{entry.score}</Text>
                  </div>
                </Box>
              </Box>
            </Card>
