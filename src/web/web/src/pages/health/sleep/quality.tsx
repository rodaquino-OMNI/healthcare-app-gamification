        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
          {QUALITY_OPTIONS.map((opt) => (
            <div
              ...
              tabIndex={0}
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setSelected(opt.value); }}
              style={{
                cursor: 'pointer', textAlign: 'center', padding: spacing.xs,
                borderRadius: '12px',
                backgroundColor: selected === opt.value ? `${colors.journeys.health.primary}22` : 'transparent',
              }}
            >
              <Text fontSize="2xl">{opt.emoji}</Text>
              <Text fontSize="xs" color={selected === opt.value ? colors.journeys.health.primary : colors.gray[40]}>{opt.label}</Text>
            </div>
          ))}
        </Box>
      </Card>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm, marginBottom: spacing['2xl'] }}>
        {FACTORS.map((factor) => (
          <div
            ...
            tabIndex={0}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') toggleFactor(factor.id); }}
            ...
            <Card journey="health" elevation="sm" padding="md">
              <Box display="flex" alignItems="center" style={{ gap: spacing.sm }}>
                <div style={{
                  ...
                  backgroundColor: factors.includes(factor.id) ? colors.journeys.health.primary : 'transparent',
                }}>
