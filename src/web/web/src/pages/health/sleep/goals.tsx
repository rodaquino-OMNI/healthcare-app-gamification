    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.lg }}>
        <Box display="flex" style={{ gap: spacing.md }}>
          <Box style={{ flex: 1 }}>
            <Text fontSize="sm" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>Target Bedtime</Text>
            <input type="time" value={targetBedtime} onChange={(e) => setTargetBedtime(e.target.value)} aria-label="Target bedtime" style={inputStyle} />
          </Box>
          <Box style={{ flex: 1 }}>
            <Text fontSize="sm" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>Target Wake Time</Text>
            <input type="time" value={targetWake} onChange={(e) => setTargetWake(e.target.value)} aria-label="Target wake time" style={inputStyle} />
          </Box>
        </Box>
      </Card>

      <Card journey="health" elevation="sm" padding="lg" style={{ marginBottom: spacing.lg, textAlign: 'center' }}>
        <Text fontSize="sm" color={colors.gray[50]}>Target Duration</Text>
        <Text fontSize="2xl" fontWeight="bold" color={colors.journeys.health.primary} style={{ margin: `${spacing.xs} 0` }}>
          {durationHours}h {durationMins}m
        </Text>
        <Text fontSize="xs" color={colors.gray[40]}>Recommended: 7-9 hours</Text>
      </Card>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing['2xl'] }}>
        <div
          ...
          tabIndex={0}
          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setNotifications(!notifications); }}
          style={{ cursor: 'pointer' }}
        >
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Box>
              <Text fontSize="md" fontWeight="semiBold">Bedtime Reminders</Text>
              <Text fontSize="sm" color={colors.gray[50]}>Get notified 30 minutes before bedtime</Text>
            </Box>
            <div style={{
