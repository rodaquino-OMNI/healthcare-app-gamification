    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.lg }}>
        <Box display="flex" style={{ gap: spacing.md }}>
          <Box style={{ flex: 1 }}>
            <Text fontSize="sm" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>Bedtime</Text>
            <input type="time" value={bedtime} onChange={(e) => setBedtime(e.target.value)} aria-label="Bedtime" style={inputStyle} />
          </Box>
          <Box style={{ flex: 1 }}>
            <Text fontSize="sm" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>Wake Time</Text>
            <input type="time" value={wakeTime} onChange={(e) => setWakeTime(e.target.value)} aria-label="Wake time" style={inputStyle} />
          </Box>
        </Box>
      </Card>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
          {STAR_LABELS.map((label, i) => (
            <div
              ...
              aria-label={label}
              tabIndex={0}
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setRating(i + 1); }}
              style={{ cursor: 'pointer', textAlign: 'center' }}
            >
              <Text fontSize="2xl">{rating >= i + 1 ? '\u2605' : '\u2606'}</Text>
              <Text fontSize="xs" color={colors.gray[40]}>{label}</Text>
            </div>
          ))}
        </Box>
      </Card>
