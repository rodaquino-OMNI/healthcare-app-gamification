...
const DATA_SOURCES = [
  ...
  { id: 'manual', label: 'Manual Entry', description: 'Log sleep data manually' },
];
  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Text fontSize="md" color={colors.gray[50]} style={{ marginTop: spacing.xs, marginBottom: spacing.xl }}>
        Connect a device or app to import sleep data
      </Text>
        ...
        {connected ? (
          <Box>
            <Box display="flex" justifyContent="space-between" alignItems="center">
              <Box>
                <Text fontWeight="semiBold" fontSize="md">{DATA_SOURCES.find((s) => s.id === source)?.label}</Text>
                <Text fontSize="sm" color={colors.semantic.success}>Connected</Text>
              </Box>
              <div style={{
            ...
            <Text fontSize="xs" color={colors.gray[40]}>Last synced: Today at 08:15 AM</Text>
          </Box>
        ) : (
          <Box style={{ textAlign: 'center', padding: spacing.lg }}>
            <Text fontSize="md" color={colors.gray[40]}>No device connected</Text>
            <Text fontSize="sm" color={colors.gray[40]} style={{ marginTop: spacing['3xs'] }}>Select a data source below to get started</Text>
          </Box>
        )}
      </Card>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm, marginBottom: spacing['2xl'] }}>
        {DATA_SOURCES.map((opt) => (
          <div key={opt.id} onClick={() => setSource(opt.id)} role="radio" aria-checked={source === opt.id} tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setSource(opt.id); }} style={{ cursor: 'pointer' }}>
      ...
      <Button journey="health" onPress={handleConnect} accessibilityLabel={connected ? 'Disconnect device' : 'Connect device'}>
