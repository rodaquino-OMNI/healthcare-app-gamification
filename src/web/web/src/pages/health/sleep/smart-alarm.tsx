    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
        ...
        {WINDOW_OPTIONS.map((opt) => (
          <div key={opt.id} style={{ flex: 1 }}>
            <div
              ...
              tabIndex={0}
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setWakeWindow(opt.id); }}
              style={{ cursor: 'pointer' }}
            >
              <Card journey="health" elevation="sm" padding="md" style={{
                textAlign: 'center',
                border: wakeWindow === opt.id ? `2px solid ${colors.journeys.health.primary}` : `2px solid transparent`,
              }}>
                <Text fontSize="md" fontWeight={wakeWindow === opt.id ? 'bold' : 'regular'} color={wakeWindow === opt.id ? colors.journeys.health.primary : colors.gray[60]}>
                  {opt.label}
                </Text>
              </Card>
            </div>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.xl }}>
        <div
          ...
          tabIndex={0}
          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setVibration(!vibration); }}
          style={{ cursor: 'pointer' }}
        >
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Box>
              <Text fontSize="md" fontWeight="semiBold">Vibration</Text>
              <Text fontSize="sm" color={colors.gray[50]}>Gentle vibration before alarm sound</Text>
            </Box>
            <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm, marginBottom: spacing['2xl'] }}>
        {SOUND_OPTIONS.map((opt) => (
          <div key={opt.id} onClick={() => setSound(opt.id)} role="radio" aria-checked={sound === opt.id} tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setSound(opt.id); }} style={{ cursor: 'pointer' }}>
