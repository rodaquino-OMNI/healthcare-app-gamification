  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="lg" style={{ marginBottom: spacing.xl, textAlign: 'center' }}>
        <Text fontSize="sm" color={colors.gray[50]}>Sleep Score</Text>
        <Text fontSize="2xl" fontWeight="bold" color={colors.journeys.health.primary} style={{ margin: `${spacing.xs} 0` }}>
          85
        </Text>
        <Text fontSize="sm" fontWeight="semiBold" color={colors.semantic.success}>Good</Text>
      </Card>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm, marginBottom: spacing.xl }}>
        <Card journey="health" elevation="sm" padding="md">
          <Text fontSize="xs" color={colors.gray[50]}>Time in Bed</Text>
          <Text fontSize="lg" fontWeight="bold" color={colors.journeys.health.primary}>8h 00m</Text>
          <Text fontSize="xs" color={colors.gray[40]}>23:00 - 07:00</Text>
        </Card>
        <Card journey="health" elevation="sm" padding="md">
          <Text fontSize="xs" color={colors.gray[50]}>Time Asleep</Text>
          <Text fontSize="lg" fontWeight="bold" color={colors.journeys.health.primary}>7h 34m</Text>
          <Text fontSize="xs" color={colors.gray[40]}>94% efficiency</Text>
        </Card>
      </div>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.xl }}>
        <div style={{ display: 'flex', borderRadius: '8px', overflow: 'hidden', height: 24, marginBottom: spacing.sm }}>
          {STAGES.map((stage) => (
          ...
          {STAGES.map((stage) => (
            <Box key={stage.label} display="flex" justifyContent="space-between" alignItems="center">
              <Box display="flex" alignItems="center" style={{ gap: spacing.xs }}>
                <div style={{ width: 10, height: 10, borderRadius: '2px', backgroundColor: stage.color, flexShrink: 0 }} />
                <Text fontSize="sm" color={colors.gray[60]}>{stage.label}</Text>
              </Box>
              <Box display="flex" alignItems="center" style={{ gap: spacing.sm }}>
                <Text fontSize="sm" fontWeight="semiBold">{stage.hours}</Text>
                <Text fontSize="xs" color={colors.gray[40]}>{stage.pct}%</Text>
              </Box>
            </Box>
          ))}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: spacing.sm }}>
        {ENVIRONMENT.map((env) => (
          <Card key={env.label} journey="health" elevation="sm" padding="md" style={{ textAlign: 'center' }}>
            <Text fontSize="xs" color={colors.gray[50]}>{env.label}</Text>
            <Text fontSize="lg" fontWeight="bold" color={colors.journeys.health.primary}>{env.value}</Text>
          </Card>
        ))}
      </div>
    </div>
