    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.xl }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" style={{ marginBottom: spacing.md }}>
          <Box>
            <Text fontWeight="semiBold" fontSize="md">Average Cycle Length</Text>
            <Text fontSize="sm" color={colors.gray[50]}>Typical range: 21-35 days</Text>
          </Box>
          <Box display="flex" alignItems="center" style={{ gap: spacing.xs }}>
            <input type="number" value={cycleLength} onChange={(e) => setCycleLength(e.target.value)} min={21} max={45} style={inputStyle} aria-label="Cycle length in days" />
            <Text fontSize="sm" color={colors.gray[50]}>days</Text>
          </Box>
        </Box>
        <div style={{ height: 1, backgroundColor: colors.gray[10] }} />
        <Box display="flex" justifyContent="space-between" alignItems="center" style={{ marginTop: spacing.md }}>
          <Box>
            <Text fontWeight="semiBold" fontSize="md">Average Period Length</Text>
            <Text fontSize="sm" color={colors.gray[50]}>Typical range: 3-7 days</Text>
          </Box>
          <Box display="flex" alignItems="center" style={{ gap: spacing.xs }}>
            <input type="number" value={periodLength} onChange={(e) => setPeriodLength(e.target.value)} min={1} max={10} style={inputStyle} aria-label="Period length in days" />
            <Text fontSize="sm" color={colors.gray[50]}>days</Text>
          </Box>
        </Box>
      </Card>
        ...
        {NOTIFICATION_OPTIONS.map((opt) => (
          <Card key={opt.id} journey="health" elevation="sm" padding="md">
            <Box display="flex" justifyContent="space-between" alignItems="center">
              <Text fontWeight="medium" fontSize="md">{opt.label}</Text>
              <button onClick={() => toggleNotification(opt.id)} style={toggleStyle(!!notifications[opt.id])} role="switch" aria-checked={!!notifications[opt.id]} aria-label={`Toggle ${opt.label}`}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm, marginBottom: spacing['2xl'] }}>
        <Button variant="secondary" journey="health" onPress={handleExport} accessibilityLabel="Export cycle data">
          Export Data
        </Button>
        <button onClick={handleClearData} style={{ background: 'none', border: `1px solid ${colors.semantic.error}`, color: colors.semantic.error, borderRadius: '8px', padding: `${spacing.sm} ${spacing.md}`, cursor: 'pointer', fontSize: '14px', fontWeight: 600 }} aria-label="Delete all cycle data">
          Delete All Data
        </button>
      </div>
