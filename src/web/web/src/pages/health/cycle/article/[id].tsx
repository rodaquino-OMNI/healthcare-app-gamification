  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <div style={{ marginTop: spacing.lg, marginBottom: spacing.sm }}>
        <Text fontSize="xs" color={colors.journeys.health.primary} fontWeight="semiBold">{article.category}</Text>
      </div>
      ...
      <Box display="flex" style={{ gap: spacing.md, marginTop: spacing.sm, marginBottom: spacing.xl }}>
        <Text fontSize="sm" color={colors.gray[50]}>{article.author}</Text>
        <Text fontSize="sm" color={colors.gray[40]}>{article.readTime} read</Text>
      </Box>
      ...
      <Box display="flex" style={{ gap: spacing.sm, marginBottom: spacing['2xl'] }}>
        ...
        <Button variant="secondary" journey="health" onPress={handleBookmark} accessibilityLabel="Bookmark article">
          Bookmark
        </Button>
      </Box>
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm }}>
            {article.relatedIds.map((rid, i) => (
              <div key={rid} onClick={() => router.push(`/health/cycle/article/${rid}`)} style={{ cursor: 'pointer' }} role="link" tabIndex={0} aria-label={article.relatedTitles[i]} onKeyDown={(e) => { if (e.key === 'Enter') router.push(`/health/cycle/article/${rid}`); }}>
                <Card journey="health" elevation="sm" padding="md">
                  <Text fontWeight="semiBold" fontSize="md">{article.relatedTitles[i]}</Text>
                </Card>
              </div>
            ))}
          </div>
