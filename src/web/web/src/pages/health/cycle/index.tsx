    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md">
        <Box display="flex" justifyContent="space-between" alignItems="center" style={{ marginBottom: spacing.sm }}>
          <Text fontWeight="semiBold" fontSize="lg">{monthName}</Text>
          <div style={{ display: 'flex', alignItems: 'center', gap: spacing.xs }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: PHASE_COLORS[currentPhase] }} />
            <Text fontSize="sm" color={colors.gray[50]}>{PHASE_LABELS[currentPhase]}</Text>
          </div>
        </Box>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: spacing['3xs'], textAlign: 'center' }}>
          ...
          {calendarDays.map((cell) => (
            ...
            >
              {cell.day > 0 && (
                <Text fontSize="sm" color={cell.day === today.getDate() ? colors.journeys.health.primary : colors.gray[60]} fontWeight={cell.day === today.getDate() ? 'bold' : 'regular'}>
                  {cell.day}
                </Text>
              )}
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm }}>
        <Card journey="health" elevation="sm" padding="md">
          <Text fontSize="sm" color={colors.gray[50]}>Cycle Length</Text>
          <Text fontSize="xl" fontWeight="bold" color={colors.journeys.health.primary}>28 days</Text>
        </Card>
        <Card journey="health" elevation="sm" padding="md">
          <Text fontSize="sm" color={colors.gray[50]}>Period Length</Text>
          <Text fontSize="xl" fontWeight="bold" color={colors.journeys.health.primary}>5 days</Text>
        </Card>
        <Card journey="health" elevation="sm" padding="md">
          <Text fontSize="sm" color={colors.gray[50]}>Day of Cycle</Text>
          <Text fontSize="xl" fontWeight="bold" color={colors.journeys.health.primary}>Day 22</Text>
        </Card>
        <Card journey="health" elevation="sm" padding="md">
          <Text fontSize="sm" color={colors.gray[50]}>Next Period</Text>
          <Text fontSize="xl" fontWeight="bold" color={colors.journeys.health.primary}>6 days</Text>
        </Card>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: spacing.sm }}>
        {NAV_LINKS.map((link) => (
          <Button key={link.href} variant="secondary" journey="health" onPress={() => router.push(link.href)} accessibilityLabel={link.label}>
            {link.label}
          </Button>
        ))}
      </div>
