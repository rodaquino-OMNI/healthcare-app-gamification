        ...
        <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
                ...
                {Object.entries(AVERAGES).map(([key, val]) => (
                    <Card key={key} journey="health" elevation="sm" padding="md">
                        <Text fontSize="xs" color={colors.gray[50]}>
                            {key === 'cycleLength' ? 'Avg Cycle' : key === 'periodLength' ? 'Avg Period' : 'Tracked'}
                        </Text>
                ...
                {MOCK_CYCLES.map((cycle) => (
                    <Card key={cycle.id} journey="health" elevation="sm" padding="md">
                        <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                            ...
                            <Text fontSize="sm" fontWeight="semiBold" color={flowColor(cycle.flow)}>
                                {cycle.flow}
                            </Text>
                        </Box>
            ...
            <Box display="flex" justifyContent="center">
                <Button
                    variant="secondary"
                    journey="health"
                    onPress={() => void router.push('/health/cycle/export')}
