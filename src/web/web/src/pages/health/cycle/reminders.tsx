...
const RemindersPage: React.FC = () => {
  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
          ...
          <Card key={r.id} journey="health" elevation="sm" padding="md">
            <Box display="flex" justifyContent="space-between" alignItems="center">
              <Box style={{ flex: 1, marginRight: spacing.md }}>
                <Text fontWeight="semiBold" fontSize="md">{r.label}</Text>
                <Text fontSize="sm" color={colors.gray[50]}>{r.description}</Text>
              </Box>
              <button
      ...
      <div style={{ display: 'flex', borderRadius: '8px', border: `1px solid ${colors.gray[20]}`, overflow: 'hidden', marginBottom: spacing.xl }}>
        {TIMING_OPTIONS.map((opt) => (
          <button
            key={opt.id}
            onClick={() => setTiming(opt.id)}
            style={{
              flex: 1, padding: `${spacing.sm} ${spacing.xs}`, border: 'none', cursor: 'pointer', fontSize: '13px',
      ...
      <Box display="flex" style={{ flexDirection: 'column', gap: spacing.sm }}>
        <Button variant="secondary" journey="health" onPress={handleTestNotification} accessibilityLabel="Send test notification">
