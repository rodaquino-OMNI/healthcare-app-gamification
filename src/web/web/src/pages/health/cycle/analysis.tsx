  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="lg" style={{ marginBottom: spacing.xl, textAlign: 'center' }}>
        <Text fontSize="sm" color={colors.gray[50]}>Regularity Score</Text>
        <Text fontSize="2xl" fontWeight="bold" color={colors.journeys.health.primary} style={{ margin: `${spacing.xs} 0` }}>
          {REGULARITY.score}%
        </Text>
        <Text fontSize="md" fontWeight="semiBold" color={colors.journeys.health.text}>{REGULARITY.label}</Text>
        <Text fontSize="sm" color={colors.gray[50]} style={{ marginTop: spacing['3xs'] }}>{REGULARITY.description}</Text>
      </Card>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm, marginBottom: spacing.xl }}>
        {STATS.map((s) => (
          <Card key={s.label} journey="health" elevation="sm" padding="md">
            ...
            <Text fontSize="xs" color={colors.gray[40]}>{s.detail}</Text>
          </Card>
        ))}
      </div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: spacing.sm, height: 120 }}>
          {CYCLE_LENGTHS.map((len, i) => (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <Text fontSize="xs" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>{len}</Text>
              <div style={{
                width: '100%', height: `${(len / MAX_BAR) * 100}%`,
                backgroundColor: colors.journeys.health.primary, borderRadius: '4px 4px 0 0',
                minHeight: 8,
              }} />
              <Text fontSize="xs" color={colors.gray[40]} style={{ marginTop: spacing['3xs'] }}>C{i + 1}</Text>
            </div>
          ))}
        </div>
      </Card>
        ...
        {TREND_ITEMS.map((item) => (
          <Card key={item.label} journey="health" elevation="sm" padding="md">
            <Box display="flex" justifyContent="space-between" alignItems="center">
              <Text fontSize="md" color={colors.journeys.health.text}>{item.label}</Text>
              <Text fontSize="sm" fontWeight="semiBold" color={item.color}>{item.value}</Text>
            </Box>
          </Card>
        ))}
      </div>
