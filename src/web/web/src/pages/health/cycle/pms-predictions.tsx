    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.xl }}>
        {PMS_TIMELINE.map((item, idx) => (
          <Box key={item.day} display="flex" alignItems="flex-start" style={{ gap: spacing.sm, paddingBottom: idx < PMS_TIMELINE.length - 1 ? spacing.sm : 0 }}>
            <div style={{
              display: 'flex', flexDirection: 'column', alignItems: 'center',
            }}>
              <div style={{
                width: 12, height: 12, borderRadius: '50%',
                backgroundColor: item.status === 'current' ? colors.journeys.health.primary : colors.gray[20],
                border: item.status === 'current' ? `2px solid ${colors.journeys.health.secondary}` : 'none',
              }} />
            ...
            <Box>
              <Text fontSize="sm" fontWeight="semiBold" color={item.status === 'current' ? colors.journeys.health.primary : colors.gray[60]}>
                {item.day}
              </Text>
        ...
        {PREDICTED_SYMPTOMS.map((item) => (
          <Card key={item.symptom} journey="health" elevation="sm" padding="md">
            <Box display="flex" justifyContent="space-between" alignItems="center">
              <Text fontSize="md" color={colors.journeys.health.text}>{item.symptom}</Text>
              <Text fontSize="sm" fontWeight="semiBold" color={item.color}>{item.likelihood}</Text>
            </Box>
          </Card>
        ))}
      </div>
