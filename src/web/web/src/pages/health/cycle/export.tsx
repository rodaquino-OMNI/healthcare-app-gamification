...
const ExportPage: React.FC = () => {
  ...
  const handleGenerate = () => {
    window.alert(`Generating ${format.toUpperCase()} with ${selectedCount} data sections for ${startDate} to ${endDate}`);
  };
    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.xl }}>
        <Box display="flex" style={{ gap: spacing.md }}>
          <Box style={{ flex: 1 }}>
            ...
            <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} aria-label="Start date" style={{ width: '100%', padding: spacing.xs, border: `1px solid ${colors.gray[20]}`, borderRadius: '8px', fontSize: '14px', color: colors.gray[60] }} />
          </Box>
          <Box style={{ flex: 1 }}>
            <Text fontSize="sm" color={colors.gray[50]} style={{ marginBottom: spacing['3xs'] }}>To</Text>
            <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} aria-label="End date" style={{ width: '100%', padding: spacing.xs, border: `1px solid ${colors.gray[20]}`, borderRadius: '8px', fontSize: '14px', color: colors.gray[60] }} />
          </Box>
        </Box>
      </Card>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm, marginBottom: spacing.xl }}>
        {CONTENT_OPTIONS.map((opt) => (
          <div
            ...
            tabIndex={0}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') toggleContent(opt.id); }}
            ...
            <Card journey="health" elevation="sm" padding="md">
              <Box display="flex" alignItems="center" style={{ gap: spacing.sm }}>
                <div style={{
                  ...
                  backgroundColor: selectedContent.includes(opt.id) ? colors.journeys.health.primary : 'transparent',
                }}>
                  {selectedContent.includes(opt.id) && (
                    <Text fontSize="xs" color={colors.gray[0]} fontWeight="bold">&#10003;</Text>
                  )}
                </div>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing['2xl'] }}>
        <Box display="flex" justifyContent="space-between">
          <Text fontSize="sm" color={colors.gray[50]}>Sections Selected</Text>
          <Text fontSize="sm" fontWeight="semiBold">{selectedCount} of {CONTENT_OPTIONS.length}</Text>
        </Box>
