    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      {bookmarks.length === 0 ? (
        <Card journey="health" elevation="sm" padding="lg" style={{ textAlign: 'center' }}>
          ...
          <Button journey="health" onPress={() => router.push('/health/wellness-resources')} accessibilityLabel="Browse resources">
            Browse Resources
          </Button>
        </Card>
      ) : (
          ...
          {bookmarks.map((bookmark) => (
            <Card key={bookmark.id} journey="health" elevation="sm" padding="md">
              <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                <div style={{ flex: 1 }}>
                  <Box display="flex" alignItems="center" style={{ gap: spacing.xs, marginBottom: spacing['3xs'] }}>
                    <div style={{
                      padding: `2px ${spacing['3xs']}`, borderRadius: '4px',
                      backgroundColor: TYPE_COLORS[bookmark.type] || colors.gray[20],
                    }}>
                      <Text fontSize="xs" fontWeight="semiBold" color={colors.white}>{bookmark.type}</Text>
                    </div>
                    <Text fontSize="xs" color={colors.gray[40]}>{bookmark.savedAt}</Text>
                  </Box>
                  <Text fontSize="sm" fontWeight="bold" color={colors.journeys.health.text}>{bookmark.title}</Text>
                </div>
                <button
                  onClick={() => removeBookmark(bookmark.id)}
                  style={{ background: 'none', border: 'none', color: colors.semantic.error, cursor: 'pointer', fontSize: '14px', fontWeight: 500, padding: spacing['3xs'] }}
