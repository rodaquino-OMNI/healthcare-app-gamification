  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Box display="flex" style={{ gap: spacing.xs, marginBottom: spacing.xl, flexWrap: 'wrap' }}>
        {CATEGORIES.map((cat) => (
          <Button key={cat} variant={category === cat ? 'primary' : 'secondary'} journey="health" onPress={() => setCategory(cat)} accessibilityLabel={`Filter ${cat}`}>
            {cat}
          </Button>
        ))}
      </Box>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm }}>
        {filtered.map((video) => (
          <Card key={video.id} journey="health" elevation="sm" padding="sm" style={{ cursor: 'pointer' }} onClick={() => router.push(`/health/wellness-resources/video-player?id=${video.id}`)}>
            <div style={{
              ...
              <div style={{
                width: 40, height: 40, borderRadius: '50%', backgroundColor: colors.journeys.health.primary,
                display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.9,
              }}>
                <Text fontSize="md" color={colors.white}>&#9654;</Text>
              </div>
            </div>
            <Text fontSize="sm" fontWeight="bold" color={colors.journeys.health.text}>{video.title}</Text>
            <Box display="flex" justifyContent="space-between" alignItems="center" style={{ marginTop: spacing['3xs'] }}>
              <Text fontSize="xs" color={colors.journeys.health.primary}>{video.category}</Text>
              <Text fontSize="xs" color={colors.gray[40]}>{video.duration}</Text>
            </Box>
          </Card>
        ))}
      </div>
