...
const QuickRepliesPage: React.FC = () => {
  ...
  const filtered = activeCategory === 'All'
    ? QUICK_REPLIES
    : QUICK_REPLIES.filter((r) => r.category === activeCategory);
  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <div style={{ display: 'flex', gap: spacing.xs, overflowX: 'auto', marginBottom: spacing.lg, paddingBottom: spacing.xs }} role="tablist" aria-label="Reply categories">
        {CATEGORIES.map((cat) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm }}>
        {filtered.map((reply) => (
          <div key={reply.id} onClick={() => handleSelect(reply)} style={{ cursor: 'pointer' }} role="button" tabIndex={0} aria-label={reply.text} onKeyDown={(e) => { if (e.key === 'Enter') handleSelect(reply); }}>
            <Card journey="health" elevation="sm" padding="md">
              <Text fontSize="xs" fontWeight="semiBold" color={colors.journeys.health.primary} style={{ marginBottom: spacing['3xs'] }}>
                {reply.category}
              </Text>
              <Text fontSize="sm" color={colors.gray[60]}>{reply.text}</Text>
            </Card>
          </div>
        ))}
      </div>
