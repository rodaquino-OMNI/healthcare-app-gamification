...
const GOALS: Goal[] = [
  { id: '1', title: 'Walk 10,000 steps daily', category: 'Fitness', percent: 72, target: '10,000 steps', current: '7,200 steps' },
  { id: '2', title: 'Drink 2L water daily', category: 'Nutrition', percent: 85, target: '2,000 ml', current: '1,700 ml' },
  { id: '3', title: 'Sleep 8 hours nightly', category: 'Sleep', percent: 90, target: '8 hours', current: '7.2 hours avg' },
  { id: '4', title: 'Meditate 15 min daily', category: 'Mindfulness', percent: 60, target: '15 min', current: '9 min avg' },
  { id: '5', title: 'Exercise 3x per week', category: 'Fitness', percent: 100, target: '3 sessions', current: '3 sessions' },
  { id: '6', title: 'Eat 5 servings of vegetables', category: 'Nutrition', percent: 40, target: '5 servings', current: '2 servings avg' },
];
    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.lg }}>
        <Box display="flex" alignItems="center" style={{ gap: spacing.md }}>
          ...
          <Box>
            <Text fontSize="2xl" fontWeight="bold" color={colors.journeys.health.primary}>{overallPercent}%</Text>
            <Text fontSize="sm" color={colors.gray[50]}>Overall Progress</Text>
          </Box>
        </Box>
      </Card>

      <Box display="flex" style={{ gap: spacing.xs, marginBottom: spacing.lg, overflowX: 'auto', paddingBottom: spacing.xs }} role="tablist" aria-label="Goal categories">
        {categories.map((cat) => (
          <button
            key={cat}
        ...
        {filtered.map((goal) => (
          <Card key={goal.id} journey="health" elevation="sm" padding="md">
            <Box display="flex" alignItems="center" style={{ gap: spacing.md }}>
              ...
              <Box style={{ flex: 1 }}>
                <Text fontWeight="semiBold" fontSize="md">{goal.title}</Text>
                <Box display="flex" style={{ gap: spacing.sm, marginTop: spacing['3xs'] }}>
                  <Text fontSize="xs" color={CATEGORY_COLORS[goal.category]}>{goal.category}</Text>
                  <Text fontSize="xs" color={colors.gray[40]}>{goal.current} / {goal.target}</Text>
                </Box>
                <div style={{ width: '100%', height: 4, backgroundColor: colors.gray[10], borderRadius: 2, marginTop: spacing.xs }}>
                  <div style={{ width: `${goal.percent}%`, height: 4, backgroundColor: CATEGORY_COLORS[goal.category], borderRadius: 2 }} />
                </div>
              </Box>
            </Box>
          </Card>
      ...
      <Box display="flex" justifyContent="center" style={{ gap: spacing.sm }}>
        <Button variant="primary" journey="health" onPress={() => window.alert('Add goal dialog')} accessibilityLabel="Add goal">
          Add Goal
        </Button>
        <Button variant="secondary" journey="health" onPress={() => router.push('/wellness/daily-plan')} accessibilityLabel="View daily plan">
          View Daily Plan
        </Button>
      </Box>
    </div>
