    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md">
          ...
          {MOOD_OPTIONS.map((mood) => (
            <button
              ...
              style={{
                ...
                backgroundColor: selectedMood === mood.value ? colors.journeys.health.background : colors.gray[0],
                cursor: 'pointer',
                display: 'flex',
                alignItems: 'center',
        ...
        <Box display="flex" justifyContent="space-between">
          <Text fontSize="xs" color={colors.gray[40]}>Very Bad</Text>
          <Text fontSize="xs" color={colors.gray[40]}>Very Good</Text>
        </Box>
      </Card>

      <Box display="flex" style={{ gap: spacing.sm, marginTop: spacing.lg, marginBottom: spacing.lg }}>
        <Button variant="primary" journey="health" onPress={() => router.push('/wellness/chat')} accessibilityLabel="Start new chat">
          Start New Chat
        </Button>
        <Button variant="secondary" journey="health" onPress={() => router.push('/wellness/mood')} accessibilityLabel="View mood history">
          Mood History
        </Button>
      </Box>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm, marginBottom: spacing['2xl'] }}>
        {RECENT_CHATS.map((chat) => (
          <div key={chat.id} onClick={() => router.push('/wellness/chat')} style={{ cursor: 'pointer' }} role="link" tabIndex={0} aria-label={`Chat: ${chat.preview}`} onKeyDown={(e) => { if (e.key === 'Enter') router.push('/wellness/chat'); }}>
            <Card journey="health" elevation="sm" padding="md">
              <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                <Box style={{ flex: 1 }}>
                  <Text fontSize="sm" color={colors.gray[60]}>{chat.preview}</Text>
                  <Text fontSize="xs" color={colors.gray[40]} style={{ marginTop: spacing['3xs'] }}>{chat.date}</Text>
                </Box>
                <Text fontSize="xs" fontWeight="semiBold" color={colors.journeys.health.primary}>{chat.mood}</Text>
              </Box>
            </Card>
          </div>
        ))}
