...
const InsightsPage: React.FC = () => {
  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm, marginBottom: spacing['2xl'] }}>
        {metrics.map((metric) => (
          <Card key={metric.label} journey="health" elevation="sm" padding="md">
            <Text fontSize="xs" color={colors.gray[50]}>{metric.label}</Text>
            <Text fontSize="xl" fontWeight="bold" color={colors.journeys.health.primary} style={{ marginTop: spacing['3xs'] }}>
              {metric.value}
            </Text>
            <Text fontSize="xs" color={metric.trendUp ? colors.journeys.health.primary : colors.semantic.error} style={{ marginTop: spacing['3xs'] }}>
              {metric.trend}
            </Text>
          </Card>
        ))}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm, marginBottom: spacing['2xl'] }}>
        {WELLNESS_TIPS.map((tip) => (
          <div key={tip.id} onClick={() => router.push(`/wellness/tip/${tip.id}`)} style={{ cursor: 'pointer' }} role="link" tabIndex={0} aria-label={tip.title} onKeyDown={(e) => { if (e.key === 'Enter') router.push(`/wellness/tip/${tip.id}`); }}>
            <Card journey="health" elevation="sm" padding="md">
              <Text fontWeight="semiBold" fontSize="md">{tip.title}</Text>
              <Box display="flex" style={{ gap: spacing.sm, marginTop: spacing['3xs'] }}>
                <Text fontSize="xs" color={colors.journeys.health.primary}>{tip.category}</Text>
                <Text fontSize="xs" color={colors.gray[40]}>{tip.readTime} read</Text>
              </Box>
            </Card>
          </div>
        ))}
