    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.lg }}>
        <Box display="flex" justifyContent="space-between" style={{ marginBottom: spacing.md }}>
          {MOOD_SCALE.map((mood) => (
            ...
            >
              <Text fontSize="lg" fontWeight="bold" color={mood.color}>{mood.value}</Text>
            </button>
          ))}
        </Box>
        <Box display="flex" justifyContent="space-between" style={{ marginBottom: spacing.md }}>
          {MOOD_SCALE.map((mood) => (
            <Text key={mood.value} fontSize="xs" color={colors.gray[50]} style={{ width: 60, textAlign: 'center' }}>
              {mood.label}
            </Text>
          ))}
        </Box>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm, marginBottom: spacing.lg }}>
        <Card journey="health" elevation="sm" padding="md">
          <Text fontSize="xs" color={colors.gray[50]}>Weekly Average</Text>
          <Text fontSize="xl" fontWeight="bold" color={colors.journeys.health.primary}>{WEEKLY_AVERAGE.toFixed(1)}</Text>
        </Card>
        <Card journey="health" elevation="sm" padding="md">
          <Text fontSize="xs" color={colors.gray[50]}>Entries This Week</Text>
          <Text fontSize="xl" fontWeight="bold" color={colors.journeys.health.primary}>{ENTRIES_THIS_WEEK}</Text>
        </Card>
      </div>
            ...
            <Card key={entry.id} journey="health" elevation="sm" padding="md">
              <Box display="flex" justifyContent="space-between" alignItems="center">
                <Box>
                  ...
                  {entry.note && <Text fontSize="xs" color={colors.gray[50]} style={{ marginTop: spacing['3xs'] }}>{entry.note}</Text>}
                </Box>
                <Box style={{ display: 'flex', alignItems: 'center', gap: spacing.xs }}>
                  <div style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: mood.color }} />
                  <Text fontSize="sm" fontWeight="semiBold" color={mood.color}>{mood.label}</Text>
                </Box>
              </Box>
            </Card>
          );
