    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.lg }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" style={{ marginBottom: spacing.sm }}>
          <Text fontWeight="semiBold" fontSize="md">Today&apos;s Progress</Text>
          <Text fontWeight="bold" fontSize="lg" color={colors.journeys.health.primary}>{progressPercent}%</Text>
        </Box>
        <div style={{ width: '100%', height: 8, backgroundColor: colors.gray[10], borderRadius: 4 }}>
          <div style={{ width: `${progressPercent}%`, height: 8, backgroundColor: colors.journeys.health.primary, borderRadius: 4, transition: 'width 0.3s ease' }} />
        </div>
      </Card>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: spacing.xs, marginBottom: spacing.lg }}>
        {categoryCounts.map((cc) => (
          <Card key={cc.category} journey="health" elevation="sm" padding="sm">
            <Text fontSize="xs" color={CATEGORY_COLORS[cc.category]} fontWeight="semiBold">{cc.category}</Text>
            <Text fontSize="sm" fontWeight="bold" color={colors.journeys.health.text}>{cc.completed}/{cc.total}</Text>
          </Card>
        ))}
      </div>
          ...
          <Card key={task.id} journey="health" elevation="sm" padding="md">
            <Box display="flex" alignItems="center" style={{ gap: spacing.sm }}>
              <input
                ...
                style={{ width: 20, height: 20, cursor: 'pointer', accentColor: colors.journeys.health.primary }}
              />
              <Box style={{ flex: 1 }}>
                <Text fontSize="sm" fontWeight="medium" color={task.completed ? colors.gray[40] : colors.gray[70]} style={{ textDecoration: task.completed ? 'line-through' : 'none' }}>
                  {task.title}
                </Text>
                <Box display="flex" style={{ gap: spacing.sm, marginTop: spacing['3xs'] }}>
                  <Text fontSize="xs" color={CATEGORY_COLORS[task.category]}>{task.category}</Text>
                  <Text fontSize="xs" color={colors.gray[40]}>{task.time}</Text>
                </Box>
              </Box>
            </Box>
          </Card>
      ...
      <Box display="flex" justifyContent="center">
        <Button variant="secondary" journey="health" onPress={() => router.push('/wellness/goals')} accessibilityLabel="View goals">
          View Goals
        </Button>
      </Box>
    </div>
