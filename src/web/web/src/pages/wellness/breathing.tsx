  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <Card journey="health" elevation="sm" padding="md" style={{ marginBottom: spacing.lg }}>
        <Text fontWeight="semiBold" fontSize="md" style={{ marginBottom: spacing.sm }}>Duration</Text>
        <Box display="flex" style={{ gap: spacing.sm }}>
          {DURATION_OPTIONS.map((mins) => (
            <button
              key={mins}
              onClick={() => { if (!isRunning) setSelectedDuration(mins); }}
              style={{
                flex: 1,
                padding: spacing.sm,
                borderRadius: '8px',
                border: `2px solid ${selectedDuration === mins ? colors.journeys.health.primary : colors.gray[20]}`,
                backgroundColor: selectedDuration === mins ? colors.journeys.health.background : colors.gray[0],
      ...
      <Box display="flex" justifyContent="center" style={{ marginBottom: spacing.xl }}>
        <Button variant="primary" journey="health" onPress={handleStartPause} accessibilityLabel={isRunning ? 'Pause' : 'Start'}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.sm }}>
        <Card journey="health" elevation="sm" padding="md">
          <Text fontSize="xs" color={colors.gray[50]}>Sessions Completed</Text>
          <Text fontSize="xl" fontWeight="bold" color={colors.journeys.health.primary}>{sessionsCompleted}</Text>
        </Card>
        <Card journey="health" elevation="sm" padding="md">
          <Text fontSize="xs" color={colors.gray[50]}>Time Elapsed</Text>
