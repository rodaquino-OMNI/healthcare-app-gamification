  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: spacing.xl }}>
      ...
      <div style={{ marginTop: spacing.lg, marginBottom: spacing.sm }}>
        <Text fontSize="xs" color={colors.journeys.health.primary} fontWeight="semiBold">{tip.category}</Text>
      </div>
      ...
      <Box display="flex" style={{ gap: spacing.md, marginTop: spacing.sm, marginBottom: spacing.xl }}>
        <Text fontSize="sm" color={colors.gray[50]}>Wellness Tip</Text>
        <Text fontSize="sm" color={colors.gray[40]}>{tip.readTime} read</Text>
      </Box>
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm }}>
            {tip.relatedIds.map((rid, i) => (
              <div key={rid} onClick={() => router.push(`/wellness/tip/${rid}`)} style={{ cursor: 'pointer' }} role="link" tabIndex={0} aria-label={tip.relatedTitles[i]} onKeyDown={(e) => { if (e.key === 'Enter') router.push(`/wellness/tip/${rid}`); }}>
                <Card journey="health" elevation="sm" padding="md">
                  <Text fontWeight="semiBold" fontSize="md">{tip.relatedTitles[i]}</Text>
                  <Text fontSize="xs" color={colors.gray[50]} style={{ marginTop: spacing['3xs'] }}>Read More</Text>
                </Card>
              </div>
            ))}
          </div>
