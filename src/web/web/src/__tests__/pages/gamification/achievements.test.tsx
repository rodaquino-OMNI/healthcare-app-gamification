  ...
  useGameProfile: () => ({
    data: {
      gameProfile: {
        ...
        xp: 2500,
        achievements: [
          { id: 'a1', title: 'First Steps', journey: 'health', description: 'Complete your profile', icon: 'star', completed: true, progress: 1, total: 1 },
          { id: 'a2', title: 'Care Champion', journey: 'care', description: 'Schedule 3 appointments', icon: 'calendar', completed: false, progress: 2, total: 3 },
        ],
        quests: [
...
jest.mock('src/web/design-system/src/gamification/XPCounter', () => ({
  XPCounter: ({ value, size }: any) => (
    <div data-testid="xp-counter" data-value={value} data-size={size}>{value} XP</div>
  ),
}));
