...
const babelJestPath = path.resolve(__dirname, 'node_modules/babel-jest');
...
function likelyHasFlowTypes(source, sourcePath) {
  ...
  if (sourcePath.includes('node_modules/react-native/') ||
      sourcePath.includes('node_modules/@react-native/')) {
    return true;
  }
...
const jestHoistTransformer = babelJest.createTransformer({
  ...
  presets: [require.resolve('./node_modules/babel-preset-jest')],
  babelrc: false,
  configFile: false,
});
