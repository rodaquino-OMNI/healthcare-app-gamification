...
import ProfileVariant1 from '../ProfileVariant1';
...
describe('ProfileVariant1 (Health Info)', () => {
  ...
  it('displays blood type selection chips', () => {
    const { getByTestId } = renderComponent();
    expect(getByTestId('blood-type-A+')).toBeTruthy();
    expect(getByTestId('blood-type-O-')).toBeTruthy();
  });

  it('displays allergies input field', () => {
    const { getByTestId } = renderComponent();
    expect(getByTestId('profile-health-allergies')).toBeTruthy();
  });

  it('displays chronic condition checkboxes', () => {
    const { getByTestId } = renderComponent();
    expect(getByTestId('condition-diabetes')).toBeTruthy();
    expect(getByTestId('condition-hypertension')).toBeTruthy();
    expect(getByTestId('condition-none')).toBeTruthy();
  });

  it('displays continue and skip buttons', () => {
    const { getByTestId } = renderComponent();
    expect(getByTestId('profile-health-continue')).toBeTruthy();
    expect(getByTestId('profile-health-skip')).toBeTruthy();
  });
});
