...
jest.mock('../../../../../../design-system/src/tokens/typography', () => ({
  typography: {
    ...
    fontSize: { xs: '12px', sm: '14px', md: '16px', lg: '18px', 'text-xs': '12px', 'text-sm': '14px', 'text-md': '16px', 'heading-xl': '30px' },
...
describe('ProfileAddress', () => {
  ...
  it('displays the CEP input field', () => {
    const { getByTestId } = renderComponent();
    expect(getByTestId('profile-address-cep')).toBeTruthy();
  });

  it('displays the street input field', () => {
    const { getByTestId } = renderComponent();
    expect(getByTestId('profile-address-street')).toBeTruthy();
  });

  it('displays the continue button', () => {
    const { getByTestId } = renderComponent();
    expect(getByTestId('profile-address-continue')).toBeTruthy();
  });

  it('displays Brazilian state chips', () => {
    const { getByTestId } = renderComponent();
    expect(getByTestId('state-SP')).toBeTruthy();
    expect(getByTestId('state-RJ')).toBeTruthy();
  });
});
