...
import ProfileVariant2 from '../ProfileVariant2';
...
describe('ProfileVariant2 (Insurance Info)', () => {
    ...
    it('displays the insurance toggle', () => {
        const { getByTestId } = renderComponent();
        expect(getByTestId('profile-insurance-toggle')).toBeTruthy();
    });

    it('displays insurance form fields when hasInsurance is true', () => {
        const { getByTestId } = renderComponent();
        expect(getByTestId('profile-insurance-provider')).toBeTruthy();
        expect(getByTestId('profile-insurance-plan-number')).toBeTruthy();
        expect(getByTestId('profile-insurance-group-number')).toBeTruthy();
    });

    it('displays plan type selection chips', () => {
        const { getByTestId } = renderComponent();
        expect(getByTestId('plan-type-basic')).toBeTruthy();
        expect(getByTestId('plan-type-standard')).toBeTruthy();
        expect(getByTestId('plan-type-premium')).toBeTruthy();
    });

    it('displays the continue button', () => {
        const { getByTestId } = renderComponent();
        expect(getByTestId('profile-insurance-continue')).toBeTruthy();
    });
});
