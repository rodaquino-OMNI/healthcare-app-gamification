...
const ProfileConfirmation: React.FC = () => {
    ...
    const handleContinueToHome = (): void => {
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1 }}>
                <ContentWrapper>
                    ...
                    <SuccessIconContainer>
                        <SuccessIcon accessibilityLabel="Success checkmark">&#10003;</SuccessIcon>
                    </SuccessIconContainer>

                    {/* Title */}
                    <Title>{t('profileSetup.confirmation.title')}</Title>
                    <SubtitleText>{t('profileSetup.confirmation.subtitle')}</SubtitleText>

                    {/* Summary Card */}
                    <SummaryCard>
                        <SummaryRow>
                            <SummaryLabel>{t('common.labels.name')}</SummaryLabel>
                            <SummaryValue numberOfLines={1} testID="profile-confirm-name">
                                {profileData.name}
                            </SummaryValue>
                        </SummaryRow>
                        <SummaryRow>
                            <SummaryLabel>{t('common.labels.email')}</SummaryLabel>
                            <SummaryValue numberOfLines={1} testID="profile-confirm-email">
                                {profileData.email}
                            </SummaryValue>
                        </SummaryRow>
                        <SummaryRowLast>
                            <SummaryLabel>{t('common.labels.phone')}</SummaryLabel>
                            <SummaryValue numberOfLines={1} testID="profile-confirm-phone">
                                {profileData.phone}
                            </SummaryValue>
                        </SummaryRowLast>
                    </SummaryCard>
                    ...
                    <CelebrationText>{t('profileSetup.confirmation.welcome')}</CelebrationText>
                </ContentWrapper>
            </ScrollView>
        </Container>
    );
