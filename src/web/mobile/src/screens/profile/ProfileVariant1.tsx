...
const createHealthInfoSchema = (t: (key: string, options?: Record<string, unknown>) => string) =>
    z.object({
        bloodType: z.string().min(1, t('common.validation.required')),
...
const CheckboxBox = styled.View<{ checked: boolean }>`
    ...
    border-width: 2px;
    border-color: ${(props) => (props.checked ? colors.brand.primary : colors.gray[30])};
    background-color: ${(props) => (props.checked ? colors.brand.primary : colors.neutral.white)};
    align-items: center;
...
const ProfileVariant1: React.FC = () => {
    ...
    const {
        ...
        resolver: zodResolver(createHealthInfoSchema(t as (key: string, options?: Record<string, unknown>) => string)),
        mode: 'onBlur',
        defaultValues: {
            bloodType: '',
            allergies: '',
    ...
    const onSubmit = async (data: HealthInfoFormData): Promise<void> => {
                ...
                <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                    <ContentWrapper>
                        ...
                        <HeaderSection>
                            ...
                            <Subtitle>{t('profile.healthInfo.subtitle')}</Subtitle>
                        ...
                        <FieldContainer>
                            ...
                            <BloodTypeGrid>
                                {BLOOD_TYPE_OPTIONS.map((type) => (
                                    ...
                                    >
                                        <BloodTypeText selected={bloodType === type}>{type}</BloodTypeText>
                                    </BloodTypeChip>
                                ))}
                            </BloodTypeGrid>
                            {errors.bloodType && <ErrorText>{errors.bloodType.message}</ErrorText>}
                        </FieldContainer>
                        ...
                        <FieldContainer>
                            ...
                            {CHRONIC_CONDITIONS.map((condition) => (
                                ...
                                >
                                    <CheckboxBox checked={selectedConditions.includes(condition.id)}>
