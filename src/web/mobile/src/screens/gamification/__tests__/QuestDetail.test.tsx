...
describe('QuestDetail', () => {
    it('renders without crashing', () => {
        const { toJSON } = render(<QuestDetail />);
        expect(toJSON()).not.toBeNull();
    });

    it('displays the quest title', () => {
        const { getByText } = render(<QuestDetail />);
        expect(getByText('Morning Health Check')).toBeTruthy();
    });

    it('displays progress section label', () => {
        const { getByText } = render(<QuestDetail />);
        expect(getByText('gamification.questDetail.progress')).toBeTruthy();
    });
});
