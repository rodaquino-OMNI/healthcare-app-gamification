 ...
 */
export function useQuestFilters(quests: CategorizedQuest[], t: (key: string) => string): QuestFiltersResult {
    ...
    const sections = useMemo(() => {
        ...
        if (daily.length > 0) {
            result.push({ title: t('gamification.quests.sectionDaily'), data: daily });
        }
        if (weekly.length > 0) {
            result.push({ title: t('gamification.quests.sectionWeekly'), data: weekly });
        }
        if (special.length > 0) {
            result.push({ title: t('gamification.quests.sectionSpecial'), data: special });
        }
        return result;
    }, [filteredQuests, t]);
