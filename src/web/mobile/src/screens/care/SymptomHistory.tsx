...
const SymptomHistory: React.FC = () => {
    ...
    const filteredChecks = useMemo(() => {
        ...
        if (!option || option.days === Infinity) {
            return MOCK_PAST_CHECKS;
        }
    ...
    const renderCheckItem = ({ item, index }: { item: PastCheck; index: number }): React.ReactElement | null => (
        <Touchable
        ...
        >
            <Card journey="care" elevation="sm">
                <View style={styles.checkHeader}>
                    <Text fontSize="text-sm" color={colors.neutral.gray600} testID={`check-date-${index}`}>
                        {item.date}
                    </Text>
                    <Badge
                        variant="status"
                ...
                <Text fontSize="heading-md" fontWeight="semiBold" journey="care" testID={`check-symptom-${index}`}>
                    {item.primarySymptom}
                </Text>
    ...
    return (
        <View style={styles.root}>
            <Text variant="heading" journey="care" testID="history-title" style={styles.title}>
                {t('journeys.care.symptomChecker.history.title')}
            </Text>

            {/* Filter tabs */}
            <View style={styles.filterContainer}>{FILTER_OPTIONS.map(renderFilterTab)}</View>
