...
const SymptomBodyMapBack: React.FC = () => {
  ...
  const handleContinue = () => {
    const selectedRegionData = BACK_BODY_REGIONS.filter((r) =>
        ...
        {selectedRegions.length > 0 && (
          <View style={styles.selectedSummary}>
            <Text variant="body" fontWeight="semiBold" journey="care">
              {t('journeys.care.symptomChecker.bodyMapBack.selectedAreas', { count: selectedRegions.length })}
            </Text>
                ...
                return (
                  <View key={regionId} style={styles.chip}>
                    <Text fontSize="text-sm" color={colors.neutral.white}>
                      {region ? t(`journeys.care.symptomChecker.bodyMapBack.regions.${region.labelKey}`) : regionId}
                    </Text>
                  </View>
                );
              })}
