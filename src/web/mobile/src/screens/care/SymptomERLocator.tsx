...
const SymptomERLocator: React.FC = () => {
    ...
    const handleCall = (er: EmergencyRoom): void => {
    ...
    const handleDirections = (er: EmergencyRoom): void => {
        ...
        if (url) {
            Linking.openURL(url).catch(() => {
                Linking.openURL(`https://maps.google.com/?q=${er.latitude},${er.longitude}`);
            });
        }
    };
    ...
    const renderERItem = ({ item, index }: { item: EmergencyRoom; index: number }): React.ReactElement | null => (
        <Card journey="care" elevation="sm">
            <View style={styles.erHeader}>
                <Text fontSize="heading-md" fontWeight="semiBold" journey="care" testID={`er-name-${index}`}>
                    {item.name}
                </Text>
                <Badge
                    variant="status"
    ...
    return (
        <View style={styles.root}>
            ...
            <Text variant="heading" journey="care" testID="er-locator-title" style={styles.title}>
                {t('journeys.care.symptomChecker.erLocator.title')}
            </Text>

            <Text variant="body" journey="care" style={styles.subtitle}>
