...
const PaymentReceipt: React.FC = () => {
  ...
  const { appointmentId, paymentId } = route.params || {
  ...
  const handleEmailReceipt = useCallback(() => {
    Alert.alert(
      t('journeys.care.payment.emailSentTitle'),
      t('journeys.care.payment.emailSentMessage'),
  ...
  const handlePrintReceipt = useCallback(() => {
    Alert.alert(
      t('journeys.care.payment.printTitle'),
      t('journeys.care.payment.printMessage'),
      [{ text: t('journeys.care.payment.ok') }],
    );
  }, [t]);
