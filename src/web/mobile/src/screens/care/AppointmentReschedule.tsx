...
export const AppointmentReschedule: React.FC = () => {
  ...
  const { appointmentId, doctorId } = route.params as RouteParams;
          ...
          <View style={styles.dayLabelsRow}>
            {dayLabels.map((lbl, i) => (
              <View key={`lbl-${i}`} style={styles.dayCell}>
                <Text fontSize="sm" fontWeight="bold" color={colors.neutral.gray500}>{lbl}</Text>
              </View>
            ))}
          </View>
          <View style={styles.calendarGrid}>
            {calendarDays.map((day, idx) => {
              ...
              return (
                <TouchableOpacity
                  key={`day-${idx}`}
                  style={[styles.dayCell, isSelected && styles.daySelected, isPast && styles.dayDisabled]}
                  onPress={() => { if (day && !isPast) setSelectedDay(day); }}
                  disabled={!day || isPast}
                  accessibilityLabel={day ? `${t('consultation.reschedule.selectDate')} ${day}` : undefined}
                  accessibilityRole="button"
                ...
                >
                  {day && (
                    <Text
                      fontSize="sm"
                      color={isSelected ? colors.neutral.white : isPast ? colors.neutral.gray300 : colors.journeys.care.text}
                    >
                      {day}
                    </Text>
                  )}
...
const styles = StyleSheet.create({
  ...
  dayCell: {
    width: '14.28%', aspectRatio: 1, alignItems: 'center', justifyContent: 'center', borderRadius: spacingValues.xs,
  },
  ...
  timeSlot: {
    paddingVertical: spacingValues.xs, paddingHorizontal: spacingValues.md,
    borderRadius: spacingValues.xl, borderWidth: 1, borderColor: colors.journeys.care.primary,
  },
  ...
  notesInput: {
    ...
    padding: spacingValues.sm, minHeight: 80, textAlignVertical: 'top',
