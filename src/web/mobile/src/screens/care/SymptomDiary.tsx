...
const MOCK_DIARY_ENTRIES: DiaryEntry[] = [
  ...
  { id: 'd3', date: '2026-02-19', symptoms: 'Headache, light sensitivity', severity: 6, notes: 'Took ibuprofen, improved after 2h' },
  { id: 'd4', date: '2026-02-18', symptoms: 'Severe headache, nausea', severity: 7, notes: 'Had to rest in dark room' },
...
const getTrendLabel = (entries: DiaryEntry[]): { label: string; status: 'success' | 'warning' | 'error' } => {
  if (entries.length < 2) return { label: 'Insufficient Data', status: 'warning' };
  ...
  if (recent < older - 0.5) return { label: 'Improving', status: 'success' };
  if (recent > older + 0.5) return { label: 'Worsening', status: 'error' };
...
const SymptomDiary: React.FC = () => {
          ...
          >
            <View
              style={[
                ...
                newSeverity === num && styles.severityButtonActive,
              ]}
            >
              <Text
                fontSize="text-xs"
  ...
  const renderEntryItem = ({ item, index }: { item: DiaryEntry; index: number }) => (
    <Card journey="care" elevation="sm">
      <View style={styles.entryHeader}>
        <Text
          fontSize="text-sm"
          color={colors.neutral.gray600}
          testID={`entry-date-${index}`}
        >
          {item.date}
        </Text>
        <Badge
