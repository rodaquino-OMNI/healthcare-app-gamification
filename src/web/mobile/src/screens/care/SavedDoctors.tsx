...
const MOCK_SAVED: SavedDoctor[] = [
  { id: 'doc-001', name: 'Dra. Ana Carolina Silva', specialty: 'Cardiologia', rating: 4.9, nextAvailable: '05 Mar, 09:00', saved: true },
  { id: 'doc-002', name: 'Dr. Ricardo Mendes', specialty: 'Dermatologia', rating: 4.7, nextAvailable: '06 Mar, 14:00', saved: true },
  { id: 'doc-003', name: 'Dra. Juliana Costa', specialty: 'Pediatria', rating: 4.8, nextAvailable: '07 Mar, 10:30', saved: true },
  { id: 'doc-004', name: 'Dr. Fernando Alves', specialty: 'Ortopedia', rating: 4.6, nextAvailable: '08 Mar, 11:00', saved: true },
  { id: 'doc-005', name: 'Dra. Beatriz Santos', specialty: 'Ginecologia', rating: 4.8, nextAvailable: '09 Mar, 16:00', saved: true },
];
...
export const SavedDoctors: React.FC = () => {
  ...
  const filteredDoctors = useMemo(() => {
    if (!searchQuery.trim()) return doctors.filter((d) => d.saved);
  ...
  const handleToggleSave = useCallback((doctorId: string) => {
    setDoctors((prev) =>
      prev.map((d) => (d.id === doctorId ? { ...d, saved: !d.saved } : d)),
    );
  }, []);

  const handleBookNow = useCallback((doctorId: string) => {
  ...
  const renderItem = useCallback(({ item }: { item: SavedDoctor }) => (
      ...
      <View style={styles.cardRow}>
        ...
        <View style={styles.doctorInfo}>
          <View style={styles.nameRow}>
            <Text fontSize="md" fontWeight="bold" color={colors.journeys.care.text} style={styles.nameText}>
              {item.name}
            </Text>
            <TouchableOpacity
          ...
          <Badge journey="care" size="sm">{item.specialty}</Badge>
...
const styles = StyleSheet.create({
  ...
  listContent: {
    padding: spacingValues.md, paddingBottom: spacingValues['3xl'], gap: spacingValues.sm,
  },
