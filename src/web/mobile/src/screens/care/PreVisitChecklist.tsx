...
const LOGISTICS_ITEMS: ChecklistItem[] = [
    {
        id: 'log-transport',
        labelKey: 'consultation.preVisitChecklist.transportation',
        required: true,
        inPersonOnly: true,
    },
...
const filterItems = (items: ChecklistItem[], type: string): ChecklistItem[] =>
    items.filter((item) => {
        if (item.telemedicineOnly && type !== 'telemedicine') {
            return false;
        }
        if (item.inPersonOnly && type === 'telemedicine') {
            return false;
        }
        return true;
    });
...
export const PreVisitChecklist: React.FC = () => {
    ...
    const renderSection = (titleKey: string, items: ChecklistItem[], testId: string): React.ReactElement | null => (
