...
/**
 * Route parameters expected by VisitPrescriptions.
 */
type VisitPrescriptionsRouteParams = {
...
const VisitPrescriptions: React.FC = () => {
  ...
  const handleSendToPharmacy = useCallback(
    (rxId: string) => {
      setPrescriptions((prev) =>
        prev.map((rx) =>
          rx.id === rxId ? { ...rx, sentToPharmacy: true } : rx,
        ),
      );
      Alert.alert(
  ...
  const handleSendAllToPharmacy = useCallback(() => {
    setPrescriptions((prev) =>
      prev.map((rx) => ({ ...rx, sentToPharmacy: true })),
    );
    Alert.alert(
