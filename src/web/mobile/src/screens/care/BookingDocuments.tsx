...
const INITIAL_DOCUMENTS: DocumentItem[] = [
  { id: 'id-document', labelKey: 'consultation.documents.idDocument', icon: '\u{1F4C4}', required: true, status: 'pending' },
  { id: 'insurance-card', labelKey: 'consultation.documents.insuranceCard', icon: '\u{1F4B3}', required: true, status: 'pending' },
  { id: 'medical-referral', labelKey: 'consultation.documents.medicalReferral', icon: '\u{1F4DD}', required: false, status: 'pending' },
  { id: 'exam-results', labelKey: 'consultation.documents.examResults', icon: '\u{1F9EA}', required: false, status: 'pending' },
];
...
export const BookingDocuments: React.FC = () => {
  ...
  const handleUpload = useCallback((docId: string) => {
    setDocuments((prev) =>
      prev.map((doc) =>
        doc.id === docId ? { ...doc, status: 'uploaded' as DocumentStatus } : doc
      )
    );
  }, []);
