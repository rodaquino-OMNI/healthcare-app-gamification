 ...
 *
 * @returns The rendered ProviderSearchScreen component.
 */
const ProviderSearchScreen: React.FC = (): React.ReactElement => {
    ...
    const handleSearch = useCallback(async (): Promise<void> => {
        ...
        try {
            ...
            setProviders(providerList as unknown as Provider[]);
        } catch (e: unknown) {
            // LD1: Set the error state if an error occurs during the provider search.
            const message = e instanceof Error ? e.message : t('journeys.care.providerSearch.fetchError');
            setError(message);
            setProviders([]);
        } finally {
    ...
    return (
        <View style={styles.container}>
            ...
            <Button
                title={String(t('common.buttons.search'))}
                onPress={() => {
                    void handleSearch();
                }}
                disabled={loading}
