...
const CheckCircle = styled.View<{ completed: boolean }>`
    ...
    border-width: 2px;
    border-color: ${(props) => (props.completed ? colors.semantic.success : colors.gray[30])};
    background-color: ${(props) => (props.completed ? colors.semantic.success : 'transparent')};
    align-items: center;
...
const ChecklistLabel = styled.Text<{ completed: boolean }>`
    ...
    color: ${(props) => (props.completed ? colors.gray[40] : colors.neutral.gray900)};
...
export const HomeEmptyScreen: React.FC = () => {
    ...
    return (
        <Container>
            ...
            >
                <ContentContainer>
                    <WelcomeIcon accessibilityElementsHidden testID="home-empty-icon">
                        {'\uD83C\uDF1F'}
                    </WelcomeIcon>

                    <WelcomeTitle testID="home-empty-title">{t('home.empty.title')}</WelcomeTitle>

                    <WelcomeSubtitle testID="home-empty-subtitle">{t('home.empty.subtitle')}</WelcomeSubtitle>
                    ...
                    <ChecklistSection>
                        ...
                        {SETUP_CHECKLIST.map((item) => (
                            ...
                            >
                                ...
                                <ChecklistLabel completed={item.completed}>{t(item.labelKey)}</ChecklistLabel>
                                {!item.completed && <ChevronText accessibilityElementsHidden>{'\u203A'}</ChevronText>}
                            </ChecklistItem>
                        ))}
                    </ChecklistSection>
                    ...
                    <CTASection>
                        ...
                        >
                            <PrimaryButtonText>{t('home.empty.explore')}</PrimaryButtonText>
                        </PrimaryButton>
                        ...
                        >
                            <SecondaryButtonText>{t('home.empty.bookFirst')}</SecondaryButtonText>
                        </SecondaryButton>
                    </CTASection>
                </ContentContainer>
            </ScrollView>
