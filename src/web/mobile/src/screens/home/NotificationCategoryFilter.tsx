...
const FilterPill = styled.TouchableOpacity<{ active: boolean }>`
    ...
    background-color: ${(props) => (props.active ? colors.brand.primary : colors.gray[10])};
...
const FilterPillText = styled.Text<{ active: boolean }>`
    ...
    font-weight: ${typography.fontWeight.semiBold};
    color: ${(props) => (props.active ? colors.neutral.white : colors.gray[50])};
`;
...
export const NotificationCategoryFilterScreen: React.FC = () => {
    ...
    const keyExtractor = useCallback((item: CategoryNotification) => item.id, []);
    ...
    return (
        <Container>
            <Header>
                ...
                <HeaderTitle accessibilityRole="header" testID="category-filter-title">
                ...
                >
                    {FILTER_TABS.map((tab) => {
                        ...
                        return (
                            ...
                            >
                                <FilterPillText active={isActive}>{t(tab.labelKey)}</FilterPillText>
                            </FilterPill>
                        );
                    })}
                </ScrollView>
            ...
            {filteredNotifications.length === 0 ? (
                <EmptyContainer testID="category-filter-empty">
                    ...
                    <EmptyTitle>{t('notification.categoryFilter.empty')}</EmptyTitle>
                </EmptyContainer>
            ) : (
                <FlatList
