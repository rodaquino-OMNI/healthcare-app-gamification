...
export const SearchResultsScreen: React.FC = () => {
    ...
    const filteredMedications = useMemo(() => {
        const q = query.toLowerCase();
        return MOCK_MEDICATIONS.filter((m) => m.name.toLowerCase().includes(q) || m.category.toLowerCase().includes(q));
    }, [query]);
    ...
    const hasResults = useMemo(() => {
        switch (activeFilter) {
            ...
            default:
                return filteredDoctors.length > 0 || filteredMedications.length > 0 || filteredArticles.length > 0;
        }
        ...
        <SafeAreaView style={styles.safeArea}>
            ...
            {isLoading ? (
                ...
                <View style={styles.centerContainer}>
                    ...
                    <Text style={styles.emptyText}>{t('searchScreens.noResults', { query })}</Text>
                    <Text style={styles.emptySubtext}>{t('searchScreens.noResultsHint')}</Text>
                </View>
            ) : (
                        ...
                        <View style={styles.section}>
                            <View style={styles.sectionHeader}>
                                ...
                                <Text style={styles.resultCount}>
                                    {t('searchScreens.found', { count: filteredDoctors.length })}
                                </Text>
                            </View>
                    ...
                    {(activeFilter === 'all' || activeFilter === 'medications') && filteredMedications.length > 0 && (
                        <View style={styles.section}>
                            <View style={styles.sectionHeader}>
                                ...
                                <Text style={styles.resultCount}>
                                    {t('searchScreens.found', { count: filteredMedications.length })}
                                </Text>
                            </View>
                    ...
                    {(activeFilter === 'all' || activeFilter === 'articles') && filteredArticles.length > 0 && (
                        <View style={styles.section}>
                            <View style={styles.sectionHeader}>
                                ...
                                <Text style={styles.resultCount}>
                                    {t('searchScreens.found', { count: filteredArticles.length })}
                                </Text>
                            </View>
