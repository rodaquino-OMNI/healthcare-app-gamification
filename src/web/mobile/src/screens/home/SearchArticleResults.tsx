...
export const SearchArticleResults: React.FC = () => {
    ...
    const handleArticlePress = useCallback((articleId: string) => {
        ...
        if (article) {
            ...
            Alert.alert(article.title, `${article.source}\n\n${article.readTime} min de leitura`, [{ text: 'OK' }]);
        }
    }, []);
    ...
    const renderArticleItem = useCallback(
        ({ item }: { item: ArticleResult }) => (
            ...
            >
                ...
                <ArticleInfo>
                    <CategoryBadge>
                        <CategoryText testID={`article-category-${item.id}`}>{item.category}</CategoryText>
                    </CategoryBadge>
                    <ArticleTitle numberOfLines={2} testID={`article-title-${item.id}`}>
                        {item.title}
                    </ArticleTitle>
                    <SourceRow>
                        <SourceText testID={`article-source-${item.id}`}>{item.source}</SourceText>
                        <Dot>{'\u2022'}</Dot>
    ...
    const renderEmpty = useCallback(
        () => (
            <EmptyContainer>
                <EmptyText testID="article-results-empty">{t('search.articleResults.empty')}</EmptyText>
            </EmptyContainer>
        ),
        [t]
    );
    ...
    return (
        <Container>
            <Header>
                ...
                <HeaderTitle testID="article-results-title">{t('search.articleResults.title')}</HeaderTitle>
