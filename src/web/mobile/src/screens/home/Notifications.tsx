...
export const NotificationsScreen: React.FC = () => {
    ...
    const renderNotificationItem = ({ item }: { item: Notification }): React.ReactElement | null => {
        ...
        return (
            <TouchableOpacity
                style={[styles.notificationItem, { borderLeftColor: typeColor }, isRead && styles.readNotification]}
                onPress={() => handleNotificationPress(item)}
                accessibilityLabel={`Notificacao: ${item.title}`}
                accessibilityRole="button"
            >
                <View style={styles.notificationRow}>
                    {!isRead && <View style={[styles.unreadIndicator, { backgroundColor: typeColor }]} />}
                    <View style={styles.notificationContent}>
                        ...
                        <Text style={[styles.notificationTitle, isRead && styles.readText]}>{item.title}</Text>
                        <Text style={[styles.notificationBody, isRead && styles.readText]} numberOfLines={2}>
                            {item.body}
                        </Text>
                    </View>
                </View>
