...
const InteractionBadge = styled.View<{ hasInteraction: boolean }>`
    ...
    background-color: ${(props) => (props.hasInteraction ? colors.semantic.warningBg : colors.semantic.successBg)};
`;
...
const InteractionText = styled.Text<{ hasInteraction: boolean }>`
    ...
    color: ${(props) => (props.hasInteraction ? colors.semantic.warning : colors.semantic.success)};
`;
...
export const SearchMedicationResults: React.FC = () => {
    ...
    const renderMedicationItem = useCallback(
        ({ item }: { item: MedicationResult }) => (
            ...
            >
                ...
                <MedicationInfo>
                    <BrandName testID={`medication-brand-${item.id}`}>{item.brandName}</BrandName>
                    <GenericName testID={`medication-generic-${item.id}`}>{item.genericName}</GenericName>
                    <DosageText testID={`medication-dosage-${item.id}`}>{item.dosage}</DosageText>
                    <InteractionBadge hasInteraction={item.hasInteractions}>
                        <InteractionIcon>{item.hasInteractions ? '\u26A0\uFE0F' : '\u2705'}</InteractionIcon>
                        <InteractionText
    ...
    const renderEmpty = useCallback(
        () => (
            <EmptyContainer>
                <EmptyText testID="medication-results-empty">{t('search.medicationResults.empty')}</EmptyText>
            </EmptyContainer>
        ),
        [t]
    );
    ...
    return (
        <Container>
            <Header>
                ...
                <HeaderTitle testID="medication-results-title">{t('search.medicationResults.title')}</HeaderTitle>
