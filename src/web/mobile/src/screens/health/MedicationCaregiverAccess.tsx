...
export const MedicationCaregiverAccess: React.FC = () => {
    ...
    const handleToggleAccess = useCallback((caregiverId: string, newLevel: AccessLevel) => {
        setCaregivers((prev) => prev.map((c) => (c.id === caregiverId ? { ...c, accessLevel: newLevel } : c)));
    }, []);
    ...
    const handleAddCaregiver = useCallback(() => {
        Alert.alert(t('medication.caregiverAccess.addTitle'), t('medication.caregiverAccess.addMessage'));
    }, [t]);
    ...
    const renderCaregiverItem = useCallback(
        ({ item }: ListRenderItemInfo<Caregiver>) => (
            <Card journey="health" elevation="sm" padding="md">
                ...
                <View style={styles.accessToggle}>
                    ...
                    >
                        <Text
                            ...
                            color={item.accessLevel === 'view_only' ? colors.journeys.health.primary : colors.gray[50]}
                        >
                            {t('medication.caregiverAccess.viewOnly')}
                        </Text>
                    </Touchable>
                    ...
                    >
                        <Text
                            ...
                            color={
                                item.accessLevel === 'full_access' ? colors.journeys.health.primary : colors.gray[50]
                            }
                        >
                            {t('medication.caregiverAccess.fullAccess')}
                        </Text>
                ...
                >
                    <Text fontSize="sm" fontWeight="medium" color={colors.semantic.error} style={styles.revokeText}>
