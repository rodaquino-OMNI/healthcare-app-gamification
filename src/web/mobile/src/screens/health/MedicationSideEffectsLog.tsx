...
export const MedicationSideEffectsLog: React.FC = () => {
    ...
    const renderSideEffectItem = useCallback(
        ({ item }: ListRenderItemInfo<SideEffectEntry>) => {
            ...
            return (
                <Card journey="health" elevation="sm" padding="md" style={styles.entryCard}>
                    ...
                    <Text fontWeight="semiBold" fontSize="md" color={colors.neutral.gray800} style={styles.effectName}>
                        {item.effectName}
                    </Text>
                    <Text fontSize="sm" color={colors.neutral.gray600} numberOfLines={3}>
                        {item.notes}
                    </Text>
                </Card>
            );
    ...
    const keyExtractor = useCallback((item: SideEffectEntry) => item.id, []);
