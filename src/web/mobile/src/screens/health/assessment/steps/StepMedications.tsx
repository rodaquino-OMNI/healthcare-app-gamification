...
export const StepMedications: React.FC<StepProps> = ({ data, onUpdate }) => {
    ...
    const handleToggleTakes = useCallback(
        (value: boolean) => {
            ...
            if (value && medicationList.length === 0) {
                onUpdate('list', [createEmptyEntry()]);
            }
        },
    ...
    const handleRemoveMedication = useCallback(
        (index: number) => {
            onUpdate(
                'list',
                medicationList.filter((_, i) => i !== index)
            );
        },
        [medicationList, onUpdate]
    );
    ...
    const renderToggleBtn = (value: boolean, labelKey: string, testId: string): React.ReactElement | null => {
                                ...
                                {FREQUENCY_OPTIONS.map((freq) => (
                                    ...
                                    >
                                        <Text
                                            ...
                                            color={
                                                entry.frequency === freq ? colors.neutral.white : colors.neutral.gray700
                                            }
                                        >
