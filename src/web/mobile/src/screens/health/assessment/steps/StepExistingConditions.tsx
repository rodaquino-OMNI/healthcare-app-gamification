...
export const StepExistingConditions: React.FC<StepProps> = ({ data, onUpdate }) => {
    ...
    const handleToggle = useCallback(
        (condition: string) => {
            ...
            const updated = isSelected ? selected.filter((c: string) => c !== condition) : [...selected, condition];
                        ...
                        >
                            <View style={styles.chipContent}>
                                ...
                                <Text
                                    ...
                                    color={isActive ? colors.journeys.health.primary : colors.neutral.gray700}
                                >
                    ...
                    <View style={styles.tagsRow}>
                        {selected.map((key: string) => (
                            <View key={key} style={styles.tag}>
                                <Text fontSize="xs" fontWeight="medium" color={colors.neutral.white}>
                                    {t(`healthAssessment.conditions.${key}`)}
                                </Text>
                                <Touchable
                                    onPress={() => handleToggle(key)}
