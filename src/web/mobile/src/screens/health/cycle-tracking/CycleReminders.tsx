...
export const CycleReminders: React.FC = () => {
    ...
    const handleToggle = useCallback((type: ReminderType) => {
        setReminders((prev) => prev.map((r) => (r.type === type ? { ...r, enabled: !r.enabled } : r)));
    }, []);

    const handleDaysBefore = useCallback((type: ReminderType, days: number) => {
        setReminders((prev) => prev.map((r) => (r.type === type ? { ...r, daysBefore: days } : r)));
    }, []);
    ...
    const handleTestNotification = useCallback(() => {
        Alert.alert(t('journeys.health.cycle.reminders.testTitle'), t('journeys.health.cycle.reminders.testMessage'));
    }, [t]);
            ...
            >
                ...
                <View style={styles.sectionContainer}>
                    ...
                    {reminders.map((reminder) => (
                        <Card key={reminder.type} journey="health" elevation="sm" padding="md">
                            {/* Toggle Row */}
                                    ...
                                    <View style={styles.daysOptions}>
                                        {DAYS_BEFORE_OPTIONS.map((days) => (
                                            <Touchable
                                                ...
                                                accessibilityLabel={t('journeys.health.cycle.reminders.daysOption', {
                                                    count: days,
                                                })}
                                                accessibilityRole="button"
                                            ...
                                            >
                                                <Text
                                                    ...
                                                    color={
                                                        reminder.daysBefore === days ? colors.gray[0] : colors.gray[50]
                                                    }
                                                >
                                                    {days}
                                                </Text>
