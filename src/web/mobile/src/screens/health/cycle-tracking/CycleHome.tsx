...
const generateMonthDays = (year: number, month: number): CalendarDay[] => {
  ...
  const days: CalendarDay[] = Array.from({ length: daysInMonth }, (_, i) => {
    ...
    if (dayNum >= 1 && dayNum <= 5) phase = 'period';
    else if (dayNum >= 12 && dayNum <= 16) phase = 'fertile';
    else if (dayNum === 14) phase = 'ovulation';
    else if (dayNum >= 24 && dayNum <= 28) phase = 'pms';
...
export const CycleHome: React.FC = () => {
  ...
  const handleTabPress = useCallback(
    (tab: TabKey) => {
      setActiveTab(tab);
      if (tab === 'log') navigation.navigate(ROUTES.HEALTH_CYCLE_LOG_PERIOD);
      if (tab === 'insights') navigation.navigate(ROUTES.HEALTH_CYCLE_HISTORY);
      if (tab === 'settings') navigation.navigate(ROUTES.HEALTH_CYCLE_PMS);
    },
    [navigation],
  );
          ...
          {calendarDays.map((day, idx) => {
            ...
            return (
              ...
              >
                ...
                {hasPhase && (
                  <View
                    style={[styles.phaseDot, { backgroundColor: PHASE_COLORS[day.phase] }]}
                  />
                )}
              </View>
            );
