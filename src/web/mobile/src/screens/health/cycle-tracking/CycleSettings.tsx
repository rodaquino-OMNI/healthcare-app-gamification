...
export const CycleSettings: React.FC = () => {
    ...
    const adjustValue = useCallback((field: keyof CycleConfig, delta: number, min: number, max: number) => {
        setConfig((prev) => {
    ...
    const renderNumberInput = useCallback(
        (label: string, value: number, field: keyof CycleConfig, min: number, max: number, unit: string) => (
