...
export const DietaryGoals: React.FC = () => {
    ...
    const totalPct = useMemo(() => carbsPct + proteinPct + fatPct, [carbsPct, proteinPct, fatPct]);

    const adjust = useCallback(
        (setter: React.Dispatch<React.SetStateAction<number>>, current: number, delta: number) => {
                ...
                <View style={styles.sectionContainer}>
                    ...
                    <Card journey="health" elevation="sm" padding="md">
                        <View style={styles.calorieRow}>
                            <Ionicons name="flame-outline" size={22} color={colors.semantic.warning} />
                            <TextInput
                            ...
                            <Card key={macro.id} journey="health" elevation="sm" padding="md">
                                <View style={styles.macroRow}>
                                    ...
                                    <Touchable
                                        ...
                                        accessibilityLabel={t('journeys.health.nutrition.goals.decrease', {
                                            macro: t(macro.labelKey),
                                        })}
                                        accessibilityRole="button"
                                    ...
                                    <Text fontSize="md" fontWeight="bold" color={macro.color} style={styles.pctText}>
                                        {pct}%
                                    </Text>
                                    <Touchable
                                        onPress={() => adjust(setter, pct, 5)}
                                        accessibilityLabel={t('journeys.health.nutrition.goals.increase', {
                                            macro: t(macro.labelKey),
                                        })}
                                        accessibilityRole="button"
                    ...
                    <Card journey="health" elevation="sm" padding="md">
                        <View style={styles.totalRow}>
                            ...
                            <Text
                                fontSize="md"
                                fontWeight="bold"
                                color={Math.abs(totalPct - 100) <= 5 ? colors.semantic.success : colors.semantic.error}
                            >
                                {totalPct}%
                            </Text>
                        </View>
                            ...
                            <Card key={macro.id} journey="health" elevation="sm" padding="md">
                                ...
                                <View style={styles.comparisonRow}>
                                    <View style={styles.comparisonItem}>
                                        ...
                                        <Text fontSize="lg" fontWeight="bold" color={colors.gray[70]}>
                                            {macro.actual}g
                                        </Text>
                                    </View>
                                    <Ionicons name="arrow-forward" size={18} color={colors.gray[30]} />
                                    <View style={styles.comparisonItem}>
                                        <Text fontSize="xs" color={colors.gray[40]}>
