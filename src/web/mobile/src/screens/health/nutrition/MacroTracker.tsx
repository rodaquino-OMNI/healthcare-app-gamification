...
export const MacroTracker: React.FC = () => {
  ...
  const chartData = useMemo(
    () => (selectedPeriod === '7d' ? WEEKLY_DATA : MONTHLY_DATA),
    [selectedPeriod],
  );
  ...
  const maxDayTotal = useMemo(
    () =>
      chartData.reduce(
        (max, d) =>
          Math.max(max, d.carbsGrams + d.proteinGrams + d.fatGrams),
        1,
      ),
    [chartData],
  );
          ...
          >
            ...
            <View style={styles.ringLegend}>
              {MACRO_GOALS.map((macro) => {
                const percent =
                  totalMacros > 0
                    ? Math.round((macro.currentGrams / totalMacros) * 100)
                    : 0;
                return (
                ...
                return (
                  <View key={day.id} style={styles.barWrapper}>
                    <View style={[styles.stackedBarCol, { height: h }]}>
                      <View style={[styles.barSegment, { flex: day.fatGrams, backgroundColor: MACRO_COLORS.fat }]} />
                      <View style={[styles.barSegment, { flex: day.proteinGrams, backgroundColor: MACRO_COLORS.protein }]} />
                      <View style={[styles.barSegment, { flex: day.carbsGrams, backgroundColor: MACRO_COLORS.carbs }]} />
                    </View>
                    {selectedPeriod === '7d' && (
                      <Text fontSize="xs" color={colors.gray[50]}>{day.label}</Text>
                    )}
                  </View>
                );
              })}
        ...
        <View style={styles.sectionContainer}>
          ...
          <View style={styles.goalCardsContainer}>
            {MACRO_GOALS.map((macro) => {
              const percent =
                macro.goalGrams > 0
                  ? Math.min(macro.currentGrams / macro.goalGrams, 1)
                  : 0;
              return (
                <Card
                  key={macro.id}
