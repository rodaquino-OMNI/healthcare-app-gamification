...
export const MealDetail: React.FC = () => {
    ...
    const mealTypeColor = useMemo(() => MEAL_TYPE_COLORS[MOCK_MEAL.mealType], []);
        ...
        ({ item, index }: ListRenderItemInfo<Ingredient>) => (
            <View>
                <View style={styles.ingredientRow}>
                    <Ionicons name="ellipse" size={8} color={colors.journeys.health.primary} />
                ...
                {index < MOCK_MEAL.ingredients.length - 1 && <View style={styles.divider} />}
            </View>
        ),
        [t]
    );
