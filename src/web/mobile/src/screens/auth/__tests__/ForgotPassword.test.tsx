...
jest.mock('styled-components/native', () => {
    ...
    return {
        __esModule: true,
        default: styled,
        useTheme: () => ({ colors: { background: { default: '#fff' }, text: { default: '#000', muted: '#888' } } }),
    };
});
...
describe('ForgotPasswordScreen', () => {
    it('renders without crashing', () => {
        const { toJSON } = render(<ForgotPasswordScreen navigation={{ navigate: mockNavigate } as any} />);
        expect(toJSON()).not.toBeNull();
    });

    it('displays the forgot password title', () => {
        const { getByText } = render(<ForgotPasswordScreen navigation={{ navigate: mockNavigate } as any} />);
        expect(getByText('auth.forgotPassword.title')).toBeTruthy();
    });

    it('renders the email input', () => {
        const { getByTestId } = render(<ForgotPasswordScreen navigation={{ navigate: mockNavigate } as any} />);
