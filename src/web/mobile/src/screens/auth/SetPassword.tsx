...
const CriteriaIcon = styled.Text<{ met: boolean }>`
  font-size: ${fontSizeValues.sm}px;
  color: ${(props: { met: boolean }) =>
    props.met ? colors.semantic.success : colors.neutral.gray400};
...
const CriteriaText = styled.Text<{ met: boolean }>`
  font-size: ${fontSizeValues.sm}px;
  color: ${(props: { met: boolean }) =>
    props.met ? colors.neutral.gray800 : colors.neutral.gray500};
`;
      ...
      <CriteriaContainer>
        {criteriaItems.map((item) => (
          <CriteriaRow key={item.key}>
            <CriteriaIcon met={item.met}>
              {item.met ? '\u2713' : '\u25CB'}
            </CriteriaIcon>
            <CriteriaText met={item.met}>
              {item.label}
            </CriteriaText>
          </CriteriaRow>
        ))}
