...
const ResendText = styled.Text<{ disabled?: boolean }>`
    font-size: ${fontSizeValues.sm}px;
    font-weight: ${typography.fontWeight.semiBold};
    color: ${(props: { disabled?: boolean }) => (props.disabled ? colors.neutral.gray400 : colors.brand.primary)};
`;
...
export const EmailVerifyScreen: React.FC = () => {
    ...
    return (
        <Container>
            ...
            <Description>{t('auth.emailVerify.description')}</Description>
            ...
            <TimerContainer>
                {!canResend && (
                    <TimerText>{t('auth.emailVerify.resendIn', { time: formatCountdown(countdown) })}</TimerText>
                )}
            </TimerContainer>
            ...
            >
                <ResendText disabled={!canResend}>{t('auth.emailVerify.resendCode')}</ResendText>
            </ResendButton>
