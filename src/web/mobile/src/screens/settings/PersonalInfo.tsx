...
const SelectorButton = styled.TouchableOpacity<{ isActive?: boolean }>`
  ...
  border-width: 1px;
  border-color: ${(props) =>
    props.isActive ? colors.brand.primary : colors.gray[20]};
  border-radius: ${borderRadius.md};
  padding-horizontal: ${spacing.md};
  justify-content: center;
  background-color: ${colors.neutral.white};
...
export const PersonalInfoScreen: React.FC = () => {
  ...
  const genderLabel = form.gender
    ? t(GENDER_OPTIONS.find((g) => g.key === form.gender)?.labelKey || '')
    : null;
        ...
        >
          <ContentWrapper>
            ...
            <FieldContainer>
              ...
              <SelectorButton
                onPress={() => { setShowGender(!showGender); setShowBloodType(false); }}
                isActive={showGender}
                accessibilityRole="button"
            ...
            <FieldContainer>
              ...
              <SelectorButton
                onPress={() => { setShowBloodType(!showBloodType); setShowGender(false); }}
