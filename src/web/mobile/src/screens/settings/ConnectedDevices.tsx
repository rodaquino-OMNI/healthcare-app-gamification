...
const StatusDot = styled.View<{ isConnected: boolean }>`
    ...
    background-color: ${({ isConnected }) => (isConnected ? colors.semantic.success : colors.gray[30])};
...
const StatusText = styled.Text<{ isConnected: boolean }>`
    ...
    color: ${({ isConnected }) => (isConnected ? colors.semantic.success : colors.gray[40])};
`;
...
export const ConnectedDevicesScreen: React.FC = () => {
    ...
    const handleUnpair = useCallback(
        (device: ConnectedDevice) => {
            Alert.alert(t('settings.connectedDevices.confirmUnpair'), device.name, [
    ...
    const handlePairNew = useCallback(() => {
        Alert.alert(t('settings.connectedDevices.pairNew'), 'Bluetooth scanning...');
    }, [t]);
    ...
    const renderDevice = useCallback(
        ({ item }: { item: ConnectedDevice }) => (
            <DeviceCard>
                ...
                >
                    <UnpairButtonText>{t('settings.connectedDevices.unpair')}</UnpairButtonText>
                </UnpairButton>
            </DeviceCard>
        ),
        [handleUnpair, t]
            ...
            <FlatList
                ...
                ListFooterComponent={
                    <ListFooter>
                        ...
                        >
                            <PairButtonText>{t('settings.connectedDevices.pairNew')}</PairButtonText>
                        </PairButton>
                    </ListFooter>
                }
