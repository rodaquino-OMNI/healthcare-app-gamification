...
const FilterTabButton = styled.TouchableOpacity<{ isActive: boolean }>`
    ...
    border-bottom-color: ${({ isActive }) => (isActive ? colors.brand.primary : 'transparent')};
`;

const FilterTabText = styled.Text<{ isActive: boolean }>`
    font-family: ${typography.fontFamily.body};
    font-size: ${typography.fontSize['text-sm']};
    font-weight: ${({ isActive }) => (isActive ? typography.fontWeight.semiBold : typography.fontWeight.regular)};
    color: ${({ isActive }) => (isActive ? colors.brand.primary : colors.gray[50])};
`;
...
export const InsuranceDocsScreen: React.FC = () => {
    ...
    const handleDownload = useCallback(
        (doc: InsuranceDocument) => {
            Alert.alert(t('settings.insuranceDocs.download'), doc.name);
        },
        [t]
    );
            ...
            <FilterContainer>
                {FILTER_TABS.map((tab) => {
                    ...
                    return (
                        ...
                        >
                            <FilterTabText isActive={isActive}>{t(tab.labelKey)}</FilterTabText>
                        </FilterTabButton>
                    );
                })}
            </FilterContainer>
