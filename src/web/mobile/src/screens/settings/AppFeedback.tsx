...
const StarText = styled.Text<{ filled: boolean }>`
    font-size: 32px;
    color: ${(props) => (props.filled ? colors.semantic.warning : colors.gray[20])};
`;
...
const CategoryOption = styled.TouchableOpacity<{ selected: boolean }>`
    ...
    background-color: ${(props) => (props.selected ? colors.brand.primary + '15' : colors.neutral.white)};
`;
...
const CategoryText = styled.Text<{ selected: boolean }>`
    ...
    font-size: ${typography.fontSize['text-sm']};
    font-weight: ${typography.fontWeight.medium};
    color: ${(props) => (props.selected ? colors.brand.primary : colors.gray[60])};
`;
...
export const AppFeedbackScreen: React.FC = () => {
    ...
    const handleSubmit = async (): Promise<void> => {
        ...
        try {
            ...
            Alert.alert(t('settings.feedback.successTitle'), t('settings.feedback.successMessage'));
            setRating(0);
            setCategory('');
            setComment('');
        } catch (err: unknown) {
    ...
    const handleAppStoreRating = (): void => {
        const url =
            Platform.OS === 'ios'
    ...
    const handleRateOnStore = (): void => {
        Alert.alert(t('settings.feedback.rateOnStore'), t('settings.feedback.rateOnStoreConfirm'), [
    ...
    const getRatingLabel = (): string => {
        if (rating === 0) {
            return t('settings.feedback.ratingPlaceholder');
        }
                ...
                <ContentWrapper>
                    ...
                    <StarRow>
                        {[1, 2, 3, 4, 5].map((star) => (
                            ...
                            >
                                <StarText filled={rating >= star}>{rating >= star ? '\u2605' : '\u2606'}</StarText>
                            </StarButton>
                        ))}
                    </StarRow>
                    <RatingLabel testID="feedback-rating-label">{getRatingLabel()}</RatingLabel>
                        ...
                        {CATEGORY_OPTIONS.map((option) => (
                            ...
                            >
                                <CategoryText selected={category === option}>
                                    ...
                                    `settings.feedback.categoryOptions.${option.toLowerCase()}`
                                        ? t(`settings.feedback.categoryOptions.${option.toLowerCase()}`)
                                        : option}
                                </CategoryText>
                            </CategoryOption>
                    ...
                    <CommentInput
                        value={comment}
                        onChangeText={(text: string) => setComment(text.slice(0, MAX_COMMENT_LENGTH))}
                    ...
                    >
                        <PrimaryButtonText>
                            {isSubmitting ? t('settings.feedback.submitting') : t('settings.feedback.submit')}
                        </PrimaryButtonText>
                    </PrimaryButton>
