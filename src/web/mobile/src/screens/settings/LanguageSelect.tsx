...
const OptionRow = styled.TouchableOpacity<{ isSelected: boolean }>`
    ...
    background-color: ${({ isSelected }) => (isSelected ? colors.gray[5] : colors.neutral.white)};
...
const RadioOuter = styled.View<{ isSelected: boolean }>`
    ...
    border-width: 2px;
    border-color: ${({ isSelected }) => (isSelected ? colors.brand.primary : colors.gray[30])};
    align-items: center;
...
export const LanguageSelectScreen: React.FC = () => {
    ...
    const [selectedLanguage, setSelectedLanguage] = useState<LanguageCode>((i18n.language as LanguageCode) || 'pt-BR');

    const handleSave = (): void => {
        i18n.changeLanguage(selectedLanguage);
        Alert.alert(t('settings.languageSelect.title'), t('settings.languageSelect.save'));
        navigation.goBack();
    };
            ...
            >
                ...
                {LANGUAGES.map((lang) => {
                    ...
                    return (
                        ...
                        >
                            <RadioOuter isSelected={isSelected}>{isSelected && <RadioInner />}</RadioOuter>
