...
export const TermsOfServiceScreen: React.FC = () => {
    ...
    return (
        <Container>
            ...
            >
                <ContentWrapper>
                    <Title testID="terms-title">{t('settings.terms.title')}</Title>
                    <LastUpdated testID="terms-last-updated">{t('settings.terms.lastUpdated')} 01/01/2024</LastUpdated>

                    {SECTIONS.map((section) => (
                        <React.Fragment key={section.key}>
                            <SectionTitle accessibilityRole="header" testID={`terms-section-${section.key}`}>
                                {t(section.titleKey)}
                            </SectionTitle>
                            {section.paragraphs.map((paragraph, index) => (
                                <Paragraph key={`${section.key}-${index}`}>{paragraph}</Paragraph>
                            ))}
                        </React.Fragment>
                    ))}
                </ContentWrapper>
