...
// DS token mocks — exact paths used by ChangePassword.tsx
...
jest.mock('../../../constants/routes', () => ({
    ROUTES: { SETTINGS_CHANGE_PASSWORD: 'ChangePassword' },
}));
...
describe('ChangePasswordScreen', () => {
    ...
    it('renders the current password input', () => {
        ...
        expect(getByTestId('change-password-current')).toBeTruthy();
    });

    it('renders the new password input', () => {
        const { getByTestId } = render(<ChangePasswordScreen />);
        expect(getByTestId('change-password-new')).toBeTruthy();
    });

    it('renders the confirm password input', () => {
        const { getByTestId } = render(<ChangePasswordScreen />);
        expect(getByTestId('change-password-confirm')).toBeTruthy();
    });

    it('renders the save button', () => {
        const { getByTestId } = render(<ChangePasswordScreen />);
        expect(getByTestId('change-password-save')).toBeTruthy();
    });
});
