...
// DS token mocks — exact paths used by PersonalInfo.tsx
...
jest.mock('../../../../../design-system/src/tokens/typography', () => ({
  typography: {
    ...
    fontSize: { 'text-xs': '12px', 'text-sm': '14px', 'text-md': '16px', 'text-lg': '18px', 'heading-xl': '30px' },
...
jest.mock('../../../constants/routes', () => ({
  ROUTES: { SETTINGS_PERSONAL_INFO: 'PersonalInfo' },
}));

// useAuth mock — PersonalInfo uses session and getUserFromToken
...
describe('PersonalInfoScreen', () => {
  ...
  it('renders the full name input field', () => {
    const { getByTestId } = render(<PersonalInfoScreen />);
    expect(getByTestId('personal-info-fullname')).toBeTruthy();
  });

  it('renders the save button', () => {
    const { getByTestId } = render(<PersonalInfoScreen />);
    expect(getByTestId('personal-info-save')).toBeTruthy();
  });

  it('renders the cancel button', () => {
    const { getByTestId } = render(<PersonalInfoScreen />);
    expect(getByTestId('personal-info-cancel')).toBeTruthy();
  });
});
