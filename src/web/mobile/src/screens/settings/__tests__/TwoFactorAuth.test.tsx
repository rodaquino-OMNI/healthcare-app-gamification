...
// DS token mocks — exact paths used by TwoFactorAuth.tsx
jest.mock('../../../../../design-system/src/tokens/colors', () => ({
  colors: {
    brand: { primary: '#00f', primaryLight: '#ccccff' },
...
jest.mock('../../../constants/routes', () => ({
  ROUTES: { SETTINGS_TWO_FACTOR: 'TwoFactor' },
}));
...
describe('TwoFactorAuthScreen', () => {
  it('renders without crashing', () => {
    const { toJSON } = render(<TwoFactorAuthScreen />);
    expect(toJSON()).not.toBeNull();
  });

  it('renders the 2FA master toggle', () => {
    const { getByTestId } = render(<TwoFactorAuthScreen />);
    expect(getByTestId('two-factor-toggle')).toBeTruthy();
  });

  it('does not show method cards when toggle is off', () => {
    const { queryByTestId } = render(<TwoFactorAuthScreen />);
    expect(queryByTestId('two-factor-method-sms')).toBeNull();
    expect(queryByTestId('two-factor-method-authenticator')).toBeNull();
  });
});
