...
// i18n mock — LanguageSelect uses i18n.language and changeLanguage
...
// DS token mocks — exact paths used by LanguageSelect.tsx
jest.mock('../../../../../design-system/src/tokens/colors', () => ({
    colors: {
        ...
        gray: { 5: '#fafafa', 10: '#f9f9f9', 20: '#eee', 30: '#ddd', 40: '#ccc', 50: '#aaa' },
        journeys: {
...
jest.mock('../../../constants/routes', () => ({
    ROUTES: { SETTINGS_LANGUAGE: 'Language' },
}));
...
describe('LanguageSelectScreen', () => {
    it('renders without crashing', () => {
        const { toJSON } = render(<LanguageSelectScreen />);
        expect(toJSON()).not.toBeNull();
    });

    it('renders the Portuguese language option', () => {
        const { getByTestId } = render(<LanguageSelectScreen />);
        expect(getByTestId('language-option-pt-BR')).toBeTruthy();
    });

    it('renders the English language option', () => {
        const { getByTestId } = render(<LanguageSelectScreen />);
        expect(getByTestId('language-option-en-US')).toBeTruthy();
    });

    it('renders the Spanish language option', () => {
        const { getByTestId } = render(<LanguageSelectScreen />);
    ...
    it('renders the save button', () => {
        const { getByTestId } = render(<LanguageSelectScreen />);
        expect(getByTestId('language-save-button')).toBeTruthy();
    });
});
