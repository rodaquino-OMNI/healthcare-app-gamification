...
// DS token mocks — exact paths used by LiveChat.tsx
...
jest.mock('../../../../../design-system/src/tokens/typography', () => ({
  typography: {
    ...
    fontSize: { 'text-xs': '12px', 'text-sm': '14px', 'text-md': '16px', 'text-lg': '18px' },
...
// LiveChat does not import ROUTES or useNavigation — standalone chat screen
import { LiveChatScreen } from '../LiveChat';

describe('LiveChatScreen', () => {
  it('renders without crashing', () => {
    const { toJSON } = render(<LiveChatScreen />);
    expect(toJSON()).not.toBeNull();
  });

  it('renders the live chat title', () => {
    const { getByTestId } = render(<LiveChatScreen />);
    expect(getByTestId('live-chat-title')).toBeTruthy();
  });

  it('renders the messages list', () => {
    const { getByTestId } = render(<LiveChatScreen />);
    expect(getByTestId('chat-messages-list')).toBeTruthy();
  });

  it('renders initial messages', () => {
    const { getByTestId } = render(<LiveChatScreen />);
    expect(getByTestId('chat-message-1')).toBeTruthy();
    expect(getByTestId('chat-message-2')).toBeTruthy();
    expect(getByTestId('chat-message-3')).toBeTruthy();
  });
