...
// DS token mocks — exact paths used by ContactSupport.tsx
...
// Routes mock — ContactSupport navigates to HELP_CHAT
...
// Linking mock — ContactSupport uses Linking.openURL for phone/email
...
describe('ContactSupportScreen', () => {
  it('renders without crashing', () => {
    const { toJSON } = render(<ContactSupportScreen />);
    expect(toJSON()).not.toBeNull();
  });

  it('renders the contact support title', () => {
    const { getByTestId } = render(<ContactSupportScreen />);
    expect(getByTestId('contact-support-title')).toBeTruthy();
  });

  it('renders the start chat button', () => {
    const { getByTestId } = render(<ContactSupportScreen />);
    expect(getByTestId('contact-start-chat')).toBeTruthy();
  });

  it('renders the call button', () => {
    const { getByTestId } = render(<ContactSupportScreen />);
    expect(getByTestId('contact-call')).toBeTruthy();
  });

  it('renders the send email button', () => {
    const { getByTestId } = render(<ContactSupportScreen />);
    expect(getByTestId('contact-send-email')).toBeTruthy();
  });
});
