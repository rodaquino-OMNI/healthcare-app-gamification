...
const MOCK_SESSIONS: MeditationSession[] = [
    {
        id: 'med-001',
        titleKey: 'journeys.health.wellness.meditation.sessionSleepSounds',
        durationMinutes: 15,
        category: 'sleep',
        icon: '\u{1F634}',
    },
    {
        id: 'med-002',
        titleKey: 'journeys.health.wellness.meditation.sessionDeepFocus',
        durationMinutes: 10,
        category: 'focus',
        icon: '\u{1F3AF}',
    },
    {
        id: 'med-003',
        titleKey: 'journeys.health.wellness.meditation.sessionInnerCalm',
        durationMinutes: 20,
        category: 'calm',
        icon: '\u{1F54A}',
    },
    {
        id: 'med-004',
        titleKey: 'journeys.health.wellness.meditation.sessionMorningBoost',
        durationMinutes: 8,
        category: 'energy',
        icon: '\u{26A1}',
    },
    {
        id: 'med-005',
        titleKey: 'journeys.health.wellness.meditation.sessionBodyScan',
        durationMinutes: 12,
        category: 'calm',
        icon: '\u{1F9D8}',
    },
    {
        id: 'med-006',
        titleKey: 'journeys.health.wellness.meditation.sessionSleepStory',
        durationMinutes: 25,
        category: 'sleep',
        icon: '\u{1F31C}',
    },
...
export const CompanionMeditationScreen: React.FC = () => {
    ...
    const filteredSessions =
        activeCategory === 'all' ? MOCK_SESSIONS : MOCK_SESSIONS.filter((s) => s.category === activeCategory);
    ...
    const renderSessionCard = ({ item }: { item: MeditationSession }): React.ReactElement | null => {
                    ...
                    {isPlaying && (
                        <View style={styles.progressBarContainer}>
                            <View
                                style={[
                                    styles.progressBar,
                                    { width: `${progress}%`, backgroundColor: getCategoryColor(item.category) },
                                ]}
                            />
                        </View>
                    )}
    ...
    return (
        <SafeAreaView style={styles.container} testID="wellness-meditation-screen">
            <View style={styles.headerBar}>
                ...
                <Text style={styles.screenTitle}>{t('journeys.health.wellness.meditation.screenTitle')}</Text>
                    ...
                    <View style={styles.historySection}>
                        ...
                        {MOCK_HISTORY.map((entry) => (
                            <View key={entry.id} style={styles.historyItem} testID={`meditation-history-${entry.id}`}>
                                <View
                                    style={[styles.historyDot, { backgroundColor: getCategoryColor(entry.category) }]}
                                />
                                <View style={styles.historyInfo}>
                                    <Text style={styles.historyTitle}>{t(entry.titleKey)}</Text>
                                    <Text style={styles.historyMeta}>
                                        {entry.date} {'\u00B7'}{' '}
                                        {t('journeys.health.wellness.meditation.minutes', {
