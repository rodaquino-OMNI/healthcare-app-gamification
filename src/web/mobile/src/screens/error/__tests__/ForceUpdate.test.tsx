...
jest.mock('../../../../design-system/src/tokens/spacing', () => ({
  spacing: {
    ...
    lg: '20px', xl: '24px', '2xl': '32px', '3xl': '48px', '4xl': '64px',
  },
  spacingValues: {
    '4xs': 1, '3xs': 2, xs: 4, sm: 8, md: 16, lg: 20, xl: 24, '2xl': 32,
  },
}));
...
describe('ForceUpdate', () => {
  it('renders without crashing', () => {
    const { toJSON } = render(<ForceUpdate />);
    expect(toJSON()).not.toBeNull();
  });

  it('displays the force update title testID', () => {
    const { getByTestId } = render(<ForceUpdate />);
    expect(getByTestId('force-update-title')).toBeTruthy();
  });

  it('displays the update button', () => {
    const { getByTestId } = render(<ForceUpdate />);
    expect(getByTestId('force-update-button')).toBeTruthy();
  });

  it('displays current and required version testIDs', () => {
    const { getByTestId } = render(<ForceUpdate />);
    expect(getByTestId('force-update-current-value')).toBeTruthy();
    expect(getByTestId('force-update-required-value')).toBeTruthy();
  });

  it('displays whats new title', () => {
    const { getByTestId } = render(<ForceUpdate />);
    expect(getByTestId('force-update-whats-new-title')).toBeTruthy();
  });
});
