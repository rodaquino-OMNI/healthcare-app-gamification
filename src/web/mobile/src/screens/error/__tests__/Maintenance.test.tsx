...
describe('Maintenance', () => {
  it('renders without crashing', () => {
    const { toJSON } = render(<Maintenance />);
    expect(toJSON()).not.toBeNull();
  });

  it('displays the maintenance title testID', () => {
    const { getByTestId } = render(<Maintenance />);
    expect(getByTestId('maintenance-title')).toBeTruthy();
  });

  it('displays the expected back schedule label', () => {
    const { getByTestId } = render(<Maintenance />);
    expect(getByTestId('maintenance-schedule-label')).toBeTruthy();
  });

  it('displays the notify toggle', () => {
    const { getByTestId } = render(<Maintenance />);
    expect(getByTestId('maintenance-notify-toggle')).toBeTruthy();
  });

  it('displays the progress indicator', () => {
    const { getByTestId } = render(<Maintenance />);
    expect(getByTestId('maintenance-progress')).toBeTruthy();
  });
});
