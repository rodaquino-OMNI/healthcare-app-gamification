...
const ClaimSubmissionScreen: React.FC = () => {
    ...
    // Step indicator
    const renderStepIndicator = (): React.ReactElement | null => (
        <View style={styles.stepIndicator}>
            {Array.from({ length: TOTAL_STEPS - 1 }).map((_, i) => (
                <View key={i} style={styles.stepDotRow}>
                    ...
                    {i < TOTAL_STEPS - 2 && (
                        <View
                            style={[styles.stepLine, i < currentStep ? styles.stepLineActive : styles.stepLineInactive]}
                        />
                    )}
                </View>
            ))}
    ...
    // Step 0: Claim Type
    const renderTypeStep = (): React.ReactElement | null => (
    ...
    // Step 1: Details
    const renderDetailsStep = (): React.ReactElement | null => (
        <View style={styles.stepContent}>
    ...
    const renderDocumentsStep = (): React.ReactElement | null => (
        <View style={styles.stepContent}>
            ...
            <TouchableOpacity
                ...
                onPress={() => {
                    Alert.alert(
                        t('journeys.plan.claims.submission.upload'),
                        t('journeys.plan.claims.submission.uploadComingSoon')
                    );
                }}
    ...
    // Step 3: Review
    const renderReviewStep = (): React.ReactElement | null => {
    ...
    // Step 4: Success
    const renderSuccessStep = (): React.ReactElement | null => (
        <View style={styles.successContainer}>
            <View style={styles.successCircle}>
                <Text style={styles.successCheck}>{'\u2713'}</Text>
            </View>
            <Text style={styles.successTitle}>{t('journeys.plan.claims.submission.successTitle')}</Text>
            <Text style={styles.successSubtitle}>{t('journeys.plan.claims.submission.successSubtitle')}</Text>
            <Text style={styles.successClaimId}>ID: CLM-{Date.now().toString().slice(-8)}</Text>
            <TouchableOpacity
    ...
    const stepRenderers = [renderTypeStep, renderDetailsStep, renderDocumentsStep, renderReviewStep, renderSuccessStep];
        ...
        <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            ...
            {showValidationError && !isStepValid() && (
                <View style={styles.validationErrorContainer}>
                    <Text testID="claim-validation-error" style={styles.validationErrorText}>
                        {t('journeys.plan.claims.submission.validationError', {
                            defaultValue: 'Please fill in all required fields before continuing.',
                        })}
                    </Text>
                </View>
            )}
            ...
            {!isSuccess && (
                <View style={styles.navButtons}>
                    <TouchableOpacity style={styles.navBackButton} onPress={handleBack} accessibilityRole="button">
...
const _fw = typography.fontWeight;
...
const createStyles = (theme: Theme) =>
    StyleSheet.create({
        ...
        typeLabel: { flex: 1, fontSize: 16, fontWeight: '500' as const, fontFamily: font, color: plan.text },
        typeLabelSelected: { color: plan.primary, fontWeight: '600' as const },
        radioCircle: {
            width: 22,
            height: 22,
            borderRadius: 11,
        ...
        reviewValueHighlight: { fontSize: 16, fontWeight: '700' as const, fontFamily: font, color: plan.primary },
        ...
        successCheck: { fontSize: 36, color: colors.neutral.white, fontWeight: '700' as const },
        successTitle: {
            fontSize: 24,
            fontWeight: '700' as const,
            fontFamily: headFont,
        ...
        navNextText: { fontSize: 16, fontWeight: '600' as const, fontFamily: font, color: colors.neutral.white },
    });
