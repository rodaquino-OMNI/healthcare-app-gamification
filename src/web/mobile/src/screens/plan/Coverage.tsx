...
const Coverage: React.FC = () => {
    ...
    if (isLoading) {
        return (
            <View style={styles.loadingContainer}>
                ...
                <Text style={styles.loadingText}>{t('journeys.plan.coverage.loading')}</Text>
            </View>
        );
    }
    ...
    if (error) {
        return (
            <View style={styles.errorContainer}>
                <Text style={styles.errorText}>{t('journeys.plan.coverage.error')}</Text>
                <Text style={styles.errorSubText}>{t('common.errors.network')}</Text>
            </View>
        );
    }
    ...
    if (!coverage || coverage.length === 0) {
        return (
            <View style={styles.emptyContainer}>
                <Text style={styles.emptyText}>{t('journeys.plan.coverage.empty')}</Text>
            </View>
        );
    }
    ...
    return (
        <ScrollView testID="plan-coverage-back" style={styles.container}>
            <Text testID="plan-coverage-title" style={styles.title}>
                ...
                return (
                    <View key={type} style={styles.sectionCard}>
                        ...
                        >
                            ...
                            <Text style={styles.sectionHeaderText}>{typeLabels[type] || type}</Text>
                                    ...
                                    <View key={item.id} style={styles.coverageItem}>
                                        ...
                                        {item.limitations && (
                                            <View style={styles.limitationRow}>
                                                <Text style={styles.limitationLabel}>
                                                    {t('journeys.plan.coverage.limitations')}:
                                                </Text>
