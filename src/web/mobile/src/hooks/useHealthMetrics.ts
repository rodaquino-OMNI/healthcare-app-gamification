 ...
 */
/* eslint-disable @typescript-eslint/explicit-function-return-type, @typescript-eslint/explicit-module-boundary-types -- return type inferred from implementation */
export const useHealthMetrics = (
    ...
    const startISO: string | undefined = startDate ? new Date(startDate).toISOString() : undefined;
    const endISO: string | undefined = endDate ? new Date(endDate).toISOString() : undefined;
    ...
    const resolvedTypes: string[] = types.length > 0 ? types : Object.values(HealthMetricType);

    const { data, isPending, error, refetch } = useQuery<HealthMetric[], Error>({
