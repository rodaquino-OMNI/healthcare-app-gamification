...
const FileUploader: React.FC<FileUploaderProps> = ({ label, onFileSelected, journey }) => {
    ...
    const handleFileSelect = useCallback(async () => {
        try {
            ...
            if (Platform.OS === 'android') {
                const hasPermission = await checkAndroidPermissions();
                if (!hasPermission) {
                    Alert.alert('Permission Required', 'Storage permission is needed to select files.', [
                        { text: 'OK' },
                    ]);
                    setLoading(false);
                    return;
