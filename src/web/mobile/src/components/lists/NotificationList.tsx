...
export const NotificationList: React.FC<NotificationListProps> = ({
                ...
                <TouchableOpacity onPress={handlePress} style={styles.notificationItem}>
                    <Card journey={item.journey as 'health' | 'care' | 'plan' | undefined}>
                        <View style={styles.notificationHeader}>
                            ...
                            {item.status !== NotificationStatus.READ && <Badge style={styles.unreadIndicator} />}
                        </View>
                        <Text style={styles.notificationBody}>{item.body}</Text>
                        <Text style={styles.notificationTime}>{formatRelativeDate(new Date(item.createdAt))}</Text>
                    </Card>
                </TouchableOpacity>
            );
        },
    ...
    if (limitedNotifications.length === 0 && showEmptyState) {
        return (
            <EmptyState icon="notifications" title="No notifications yet" description="Check back later for updates" />
        );
    }
