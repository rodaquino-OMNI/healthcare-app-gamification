...
export const CoverageInfoCard: React.FC<CoverageInfoCardProps> = ({ coverage }) => {
    ...
    return (
        <Card journey="plan" elevation="sm" accessibilityLabel={`Coverage information for ${coverageName}`}>
            ...
            {coverage.coPayment !== undefined && (
                <CoPaymentBadge>Copagamento: R$ {coverage.coPayment.toFixed(2)}</CoPaymentBadge>
            )}
        </Card>
    );
};
