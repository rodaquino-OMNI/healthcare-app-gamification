 ...
 * ```
 */
export const BarChart: React.FC<BarChartProps> = ({ data, labels, colors: colorProp, journey = 'health', title }) => {
    ...
    const accessibleDescription = `${title ? `Bar chart titled ${title}. ` : ''}Chart displaying ${safeLabels.length} data points. ${safeLabels
        .map((label, index) => `${label}: ${safeData[index].toLocaleString()}`)
        .join('. ')}.`;
    ...
    return (
        ...
        >
            {title && (
                <Text as="h3" fontSize="lg" fontWeight="medium" journey={journey} marginBottom="md" id="chart-title">
                    {title}
                </Text>
            )}
