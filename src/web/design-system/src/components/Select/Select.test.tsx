  ...
  RadioButton: ({ label, checked, onChange, testID, ...props }: any) => (
    <label data-testid={testID || 'radio'}>
      <input
        ...
        checked={checked}
        onChange={onChange}
        data-testid={`${testID || 'radio'}-input`}
      />
      {label}
    </label>
  ),
...
describe('Select', () => {
  ...
  it('renders with label', () => {
    render(
      <Select options={defaultOptions} value="" onChange={jest.fn()} label="Choose one" />
    );
  ...
  it('displays placeholder when no value selected', () => {
    render(
      <Select options={defaultOptions} value="" onChange={jest.fn()} label="Choose" />
    );
    const texts = screen.getAllByTestId('text');
  ...
  it('displays selected option label for single select', () => {
    render(
      <Select options={defaultOptions} value="b" onChange={jest.fn()} label="Choose" />
    );
    const texts = screen.getAllByTestId('text');
  ...
  it('calls onChange when an option is selected', () => {
    ...
    render(
      <Select options={defaultOptions} value="" onChange={onChangeMock} label="Choose" />
    );
    fireEvent.click(screen.getByTestId('select-component'));
  ...
  it('renders checkboxes for multi-select in modal', () => {
    render(
      <Select options={defaultOptions} value={[]} onChange={jest.fn()} label="Choose" multiple />
    );
    fireEvent.click(screen.getByTestId('select-component'));
  ...
  it('does not open when disabled', () => {
    render(
      <Select options={defaultOptions} value="" onChange={jest.fn()} label="Choose" disabled />
    );
    const trigger = screen.getByTestId('select-component');
    fireEvent.click(trigger);
  ...
  it('renders search input when searchable', () => {
    render(
      <Select options={defaultOptions} value="" onChange={jest.fn()} label="Choose" searchable />
    );
    fireEvent.click(screen.getByTestId('select-component'));
