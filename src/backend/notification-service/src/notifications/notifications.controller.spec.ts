...
describe('NotificationsController', () => {
  ...
  describe('findAll', () => {
    it('should return all notifications for the authenticated user', async () => {
      ...
      expect(mockNotificationsService.findAllByUser).toHaveBeenCalledWith('user-1', undefined);
      expect(result).toEqual([mockNotification]);
    });
  ...
  describe('findOne', () => {
    ...
    it('should throw AppException when notification is not found', async () => {
      ...
      await expect(controller.findOne('nonexistent-id', 'user-1')).rejects.toThrow(AppException);
    });
  });
  ...
  describe('markAsRead', () => {
    ...
    it('should throw AppException when notification is not found', async () => {
      ...
      await expect(controller.markAsRead('nonexistent-id', 'user-1')).rejects.toThrow(AppException);
    });
  });
