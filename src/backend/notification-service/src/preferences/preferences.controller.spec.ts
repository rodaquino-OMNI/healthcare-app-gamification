...
describe('PreferencesController', () => {
  ...
  describe('getPreferences', () => {
    ...
    it('should return an empty array when the user has no preferences', async () => {
      ...
      const result = await controller.getPreferences({} as any, {} as any, 'user-with-no-prefs');
  ...
  describe('createPreference', () => {
    ...
    it('should use the current user ID as the owner of the preference', async () => {
      mockPreferencesService.create.mockResolvedValue({ ...mockPreference, userId: 'user-99' });
    ...
    it('should propagate errors from PreferencesService.create', async () => {
      ...
      await expect(controller.createPreference('user-1')).rejects.toThrow('Constraint violation');
    });
  });
  ...
  describe('updatePreference', () => {
    ...
    it('should pass the raw preference ID string to the service', async () => {
      ...
      expect(mockPreferencesService.update).toHaveBeenCalledWith('42', { emailEnabled: false });
    });
    ...
    it('should propagate errors from PreferencesService.update', async () => {
      ...
      await expect(controller.updatePreference('999', { smsEnabled: true } as any)).rejects.toThrow(
        'Record not found',
      );
    });
  });
