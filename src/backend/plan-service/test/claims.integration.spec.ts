...
describe('Claims Lifecycle Integration', () => {
    ...
    it('should reject invalid REJECTED -> APPROVED transition', async () => {
        mockPrisma.claim.findUnique.mockResolvedValue({ ...base, status: 'rejected' });
        await expect(svc.update(ID, { status: 'approved' } as any)).rejects.toBeInstanceOf(
            AppException
        );
    ...
    it('should complete full lifecycle: submit -> review -> approve', async () => {
        ...
        mockPrisma.claim.update.mockResolvedValue({ ...base, status: 'under_review' });
        expect((await svc.update(ID, { status: 'under_review' } as any)).status).toBe(
            'under_review'
        );
        ...
        mockPrisma.claim.update.mockResolvedValue({ ...base, status: 'approved' });
        expect((await svc.update(ID, { status: 'approved' } as any)).status).toBe('approved');
