  ...
  return Joi.object({
    ...
    server: Joi.object({
      port: Joi.number().port().default(3000)
        .description('Port number for the Plan Service'),
      host: Joi.string().hostname().default('localhost')
        .description('Host for the Plan Service'),
      cors: Joi.object({
    ...
    database: Joi.object({
      ...
      schema: Joi.string().default('plan')
        .description('Database schema for Plan service'),
      ssl: Joi.boolean().default(true)
        .description('Whether to use SSL for database connection'),
      poolSize: Joi.number().positive().default(20)
    ...
    insuranceApi: Joi.object({
      baseUrl: Joi.string().uri().required()
        .description('Base URL for insurance API'),
      apiKey: Joi.string().required()
        .description('API key for insurance API access'),
    ...
    claims: Joi.object({
      supportedDocumentTypes: Joi.array().items(Joi.string()).default(['pdf', 'jpg', 'jpeg', 'png'])
        .description('Supported document types for claim submissions'),
      maxDocumentSize: Joi.number().positive().default(10 * 1024 * 1024) // 10MB
        .description('Maximum document size in bytes'),
      maxDocumentsPerClaim: Joi.number().integer().positive().default(5)
        .description('Maximum number of documents per claim'),
      autoApprovalThreshold: Joi.number().positive().default(100)
        .description('Threshold for automatic claim approval in local currency units'),
      processingTimeEstimate: Joi.object({
        standard: Joi.number().integer().positive().default(3)
          .description('Standard processing time in days'),
        express: Joi.number().integer().positive().default(1)
          .description('Express processing time in days')
      }).default(),
      retentionPeriod: Joi.number().integer().positive().default(7 * 365) // 7 years
        .description('Document retention period in days')
    }).required(),

    // Document storage configuration
    storage: Joi.object({
      provider: Joi.string().valid('s3', 'local').default('s3')
        .description('Storage provider for documents'),
      s3: Joi.object({
        bucket: Joi.string().required()
          .description('S3 bucket name for document storage'),
        region: Joi.string().default('sa-east-1')
        ...
        endpoint: Joi.string().uri()
          .description('Custom S3 endpoint (for non-AWS S3 compatible services)'),
        pathPrefix: Joi.string().default('plan')
          .description('Path prefix for storing documents'),
    ...
    gamification: Joi.object({
      enabled: Joi.boolean().default(true)
        .description('Whether gamification is enabled for Plan service'),
      eventEndpoint: Joi.string().uri().required()
        .description('Endpoint for sending gamification events'),
      timeout: Joi.number().positive().default(5000)
        .description('Timeout for gamification event requests in milliseconds'),
      events: Joi.object({
        claimSubmitted: Joi.string().default('CLAIM_SUBMITTED')
          .description('Event name for claim submission'),
        claimApproved: Joi.string().default('CLAIM_APPROVED')
          .description('Event name for claim approval'),
        digitalCardAccessed: Joi.string().default('DIGITAL_CARD_ACCESSED')
          .description('Event name for digital card access'),
        benefitUsed: Joi.string().default('BENEFIT_USED')
          .description('Event name for benefit usage')
      }).default()
    }).required(),
    ...
    notifications: Joi.object({
      enabled: Joi.boolean().default(true)
        .description('Whether notifications are enabled for Plan service'),
      serviceEndpoint: Joi.string().uri().required()
        .description('Endpoint for notification service'),
      timeout: Joi.number().positive().default(5000)
        .description('Timeout for notification requests in milliseconds'),
      templates: Joi.object({
        claimStatus: Joi.string().default('plan-claim-status')
          .description('Template for claim status notifications'),
        claimReminder: Joi.string().default('plan-claim-reminder')
          .description('Template for claim reminder notifications'),
        benefitExpiration: Joi.string().default('plan-benefit-expiration')
    ...
    logging: Joi.object({
      ...
      destination: Joi.string().valid('stdout', 'file').default('stdout')
