...
describe('ClaimsController', () => {
  ...
  describe('create (POST /claims)', () => {
    ...
    it('should propagate service errors during claim creation', async () => {
      ...
      await expect(
        controller.create('user-id', { planId: 'plan-id', type: 'visit', amount: 100 } as any)
  ...
  describe('findAll (GET /claims)', () => {
    ...
    it('should propagate service errors', async () => {
      ...
      await expect(controller.findAll('user-id', {} as any, {} as any)).rejects.toThrow('Query error');
    });
  });
  ...
  describe('findOne (GET /claims/:id)', () => {
    ...
    it('should throw NotFoundException when claim does not exist', async () => {
      ...
      await expect(controller.findOne('user-id', 'non-existent-id')).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it('should throw ForbiddenException when claim belongs to another user', async () => {
      ...
      await expect(controller.findOne(userId, 'test-claim-id')).rejects.toBeInstanceOf(ForbiddenException);
    });
  ...
  describe('update (PUT /claims/:id)', () => {
    ...
    it('should throw ForbiddenException when updating another user claim', async () => {
      ...
      await expect(controller.update(userId, 'test-claim-id', {} as any)).rejects.toBeInstanceOf(ForbiddenException);
    });
    ...
    it('should propagate service errors during update', async () => {
      ...
      await expect(controller.update(userId, 'test-claim-id', {} as any)).rejects.toThrow('Update error');
    });
  });
  ...
  describe('remove (DELETE /claims/:id)', () => {
    ...
    it('should throw NotFoundException when claim does not exist for removal', async () => {
      ...
      await expect(controller.remove('user-id', 'non-existent-id')).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it('should throw ForbiddenException when removing another user claim', async () => {
      ...
      await expect(controller.remove(userId, 'test-claim-id')).rejects.toBeInstanceOf(ForbiddenException);
    });
    ...
    it('should propagate service errors during removal', async () => {
      ...
      await expect(controller.remove(userId, 'test-claim-id')).rejects.toThrow('Delete error');
    });
  });
});
