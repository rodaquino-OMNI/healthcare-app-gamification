...
export class ClaimsController {
    ...
    @HttpCode(HttpStatus.OK)
    @Roles('user')
    async findOne(@CurrentUser('id') userId: string, @Param('id') id: string): Promise<Claim> {
        ...
        return this.tracingService.createSpan('ClaimsController.findOne', async () => {
            ...
            if (claim.userId !== userId) {
                this.logger.warn(
                    `User ${userId} attempted to access claim ${id} belonging to another user`,
                    'ClaimsController'
                );
    ...
    async update(
        ...
        return this.tracingService.createSpan('ClaimsController.update', async () => {
            ...
            if (claim.userId !== userId) {
                this.logger.warn(
                    `User ${userId} attempted to update claim ${id} belonging to another user`,
                    'ClaimsController'
                );
    ...
    async remove(@CurrentUser('id') userId: string, @Param('id') id: string): Promise<void> {
        ...
        return this.tracingService.createSpan('ClaimsController.remove', async () => {
            ...
            if (claim.userId !== userId) {
                this.logger.warn(
                    `User ${userId} attempted to delete claim ${id} belonging to another user`,
                    'ClaimsController'
                );
