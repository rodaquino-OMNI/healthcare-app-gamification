    ...
    return {
        ...
        data: {
            ...
            anomaly: {
                ...
                message: 'BLOOD_GLUCOSE is critically high',
            },
        },
        ...overrides,
    };
...
describe('Health -> Gamification event contract', () => {
    ...
    describe('health.metric.anomaly.detected', () => {
        ...
        it('data.anomaly has severity and type matching allowed values', () => {
            const anomaly = (event.data as Record<string, unknown>).anomaly as Record<
                string,
                unknown
            >;
            expect(anomaly).toBeDefined();
            expect(anomaly.severity).toMatch(/^(INFO|WARNING|CRITICAL)$/);
            expect(anomaly.type).toMatch(
                /^(THRESHOLD_VIOLATION|STATISTICAL_ANOMALY|TREND_CHANGE)$/
            );
        });
