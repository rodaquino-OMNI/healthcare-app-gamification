...
export class HealthService {
  ...
  private validateMetricRange(type: MetricType, value: number): void {
    ...
    if (!range) return; // No range validation for unknown types
  ...
  private async getHistoricalMetrics(
    ...
    return await (prisma as any).healthMetric.findMany({
      where: {
        userId,
        type,
  ...
  private async detectAnomalies(
    ...
    if (thresholdAnomaly) return thresholdAnomaly;
  ...
  private detectThresholdAnomaly(type: MetricType, value: number): AnomalyDetection | null {
    const range = METRIC_RANGES[type];
    if (!range || (range.criticalLow === 0 && range.criticalHigh === 0)) return null;
  ...
  private detectTrendAnomaly(value: number, historicalMetrics: any[]): AnomalyDetection | null {
    if (historicalMetrics.length < 7) return null; // Need at least a week of data
   ...
   */
  private async emitMetricEvents(
    userId: string,
    metric: any,
   ...
   */
  private async checkAndUpdateHealthGoals(
    prisma: any,
    userId: string,
    metric: any
  ): Promise<void> {
    // Find active goals for this metric type
    const activeGoals = await (prisma as any).healthGoal.findMany({
      where: {
        userId,
        type: metric.type,
        status: 'ACTIVE',
    ...
    for (const goal of activeGoals) {
      ...
      await (prisma as any).healthGoal.update({
        where: { id: goal.id },
        data: {
