...
describe('Secure Axios', () => {
    ...
    describe('SSRF protection', () => {
        it('should block requests to localhost', async () => {
            ...
            await expect(instance.get('http://localhost/secret')).rejects.toThrow(
                'SSRF Protection'
            );
        });
        ...
        it('should block requests to private IP 10.x.x.x', async () => {
            ...
            await expect(instance.get('http://10.0.0.1/internal')).rejects.toThrow(
                'SSRF Protection'
            );
        });
        ...
        it('should block requests to private IP 192.168.x.x', async () => {
            ...
            await expect(instance.get('http://192.168.1.1/admin')).rejects.toThrow(
                'SSRF Protection'
            );
        });
        ...
        it('should block requests to 127.0.0.1', async () => {
            ...
            await expect(instance.get('http://127.0.0.1:8080/api')).rejects.toThrow(
                'SSRF Protection'
            );
        });
        ...
        it('should block requests to .local domains', async () => {
            ...
            await expect(instance.get('http://server.local/api')).rejects.toThrow(
                'SSRF Protection'
            );
        });
    });
