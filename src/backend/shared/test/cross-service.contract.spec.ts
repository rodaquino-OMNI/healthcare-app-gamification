...
export interface HealthEvent {
  type: string; userId: string; journey: string; timestamp: string; data: Record<string, unknown>;
}
...
export function validateHealthEvent(evt: unknown): VResult {
  if (!obj(evt)) return { valid: false, errors: ['not an object'] };
  const errors: string[] = [];
  if (!str(evt.type)) errors.push('type must be a non-empty string');
  if (typeof evt.userId !== 'string' || !UUID_RE.test(evt.userId)) errors.push('userId must be a UUID');
  if (!str(evt.journey)) errors.push('journey must be a non-empty string');
  if (typeof evt.timestamp !== 'string' || !ISO_RE.test(evt.timestamp)) errors.push('timestamp must be ISO-8601');
  if (!obj(evt.data)) errors.push('data must be an object');
...
export function validatePlanEvent(evt: unknown): VResult {
  if (!obj(evt)) return { valid: false, errors: ['not an object'] };
  const errors: string[] = [];
  if (!str(evt.eventType)) errors.push('eventType must be a non-empty string');
  if (typeof evt.timestamp !== 'string' || !ISO_RE.test(evt.timestamp)) errors.push('timestamp must be ISO-8601');
  const c = evt.claim as Record<string, unknown> | undefined;
  if (!obj(c)) { errors.push('claim must be an object'); }
  else {
    if (!str(c.id)) errors.push('claim.id must be a non-empty string');
    if (typeof c.userId !== 'string' || !UUID_RE.test(c.userId)) errors.push('claim.userId must be a UUID');
    if (!str(c.planId)) errors.push('claim.planId must be a non-empty string');
    if (!str(c.status)) errors.push('claim.status must be a non-empty string');
    if (typeof c.amount !== 'number' || c.amount < 0) errors.push('claim.amount must be non-negative');
  }
  return { valid: errors.length === 0, errors };
}

export function validateJwtPayload(payload: unknown): VResult {
  if (!obj(payload)) return { valid: false, errors: ['not an object'] };
  const errors: string[] = [];
  if (typeof payload.sub !== 'string' || !UUID_RE.test(payload.sub)) errors.push('sub must be a UUID');
  if (typeof payload.email !== 'string' || !payload.email.includes('@')) errors.push('email must be valid');
...
export class MockEventBus {
  ...
  reset(): void { this.publish.mockClear(); this.handlers.clear(); }
}
...
export function createMockHealthEvent(ov: Partial<HealthEvent> = {}): HealthEvent {
  return {
    type: 'HEALTH_METRIC_RECORDED', userId: DEFAULT_UUID, journey: 'health',
    timestamp: new Date().toISOString(), data: { metricType: 'blood_pressure', value: 120 }, ...ov,
  };
}
export function createMockPlanEvent(ov: Partial<PlanEvent> = {}): PlanEvent {
  return {
    eventType: 'CLAIM_SUBMITTED', timestamp: new Date().toISOString(),
    claim: { id: 'claim-001', userId: DEFAULT_UUID, planId: 'plan-001', status: 'pending', amount: 150.0 },
    ...ov,
  };
}
